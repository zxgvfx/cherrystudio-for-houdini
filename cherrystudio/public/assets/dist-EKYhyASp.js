import { A as record, C as string, E as union, F as unknown, j as _enum, n as any, o as array, q as boolean, u as literal, x as number, y as object } from "./v4-_VdBfIuS.js";
import { B as parseProviderOptions, D as postJsonToApi, E as removeUndefinedEntries, F as resolve, G as safeParseJSON, H as safeValidateTypes, L as withoutTrailingSlash, O as EmptyResponseBodyError, T as TooManyEmbeddingValuesForCallError, V as UnsupportedFunctionalityError, b as asSchema, c as combineHeaders, e as convertToBase64, j as createJsonErrorResponseHandler, k as createJsonResponseHandler, p as extractResponseHeaders, q as generateId, y as loadOptionalSetting, z as loadSetting } from "./dist-DljPbAPG.js";
import { c as anthropicTools, d as prepareTools } from "./internal-Dhi1dCTx.js";
import { b as Crc32, e as fromHex, f as toHex, g as toUtf8, h as fromUtf8 } from "./module-B3cvBXXU.js";
...
function prepareTools$1({ tools, toolChoice, modelId }) {
	const toolWarnings = [];
	...
	if (usingAnthropicTools) {
		...
		const { toolChoice: preparedAnthropicToolChoice, toolWarnings: anthropicToolWarnings, betas: anthropicBetas } = prepareTools({
...
var BedrockChatLanguageModel = class {
	...
	async getArgs({ prompt, maxOutputTokens, temperature, topP, topK, frequencyPenalty, presencePenalty, stopSequences, responseFormat, seed, tools, toolChoice, providerOptions }) {
		...
		if (tools != null && (responseFormat == null ? void 0 : responseFormat.type) === "json") {
			if (tools.length > 0) warnings.push({
				type: "other",
		...
		const { toolConfig, additionalTools, toolWarnings, betas } = prepareTools$1({
	...
	async doGenerate(options) {
		...
		const { command: args, warnings, usesJsonResponseTool, betas } = await this.getArgs(options);
		...
		for (const part of response.output.message.content) {
			if (part.text) {
				if (!usesJsonResponseTool) content.push({
					type: "text",
					text: part.text
				});
			}
			...
			if (part.toolUse) content.push(usesJsonResponseTool ? {
				type: "text",
		...
		const providerMetadata = response.trace || response.usage || usesJsonResponseTool ? { bedrock: {
		...
		return {
			stream: response.pipeThrough(new TransformStream({
				...
				transform(chunk, controller) {
					...
					if (value.messageStop) finishReason = mapBedrockFinishReason(value.messageStop.stopReason);
					if (value.metadata) {
						...
						if (cacheUsage || trace || usesJsonResponseTool) providerMetadata = { bedrock: {
							...cacheUsage,
							...trace,
							...usesJsonResponseTool && { isJsonResponseFromTool: true }
						} };
					...
					if (((_m = value.contentBlockDelta) == null ? void 0 : _m.delta) && "text" in value.contentBlockDelta.delta && value.contentBlockDelta.delta.text) {
						...
						if (contentBlocks[blockIndex] == null) {
							contentBlocks[blockIndex] = { type: "text" };
							if (!usesJsonResponseTool) controller.enqueue({
								type: "text-start",
								id: String(blockIndex)
							});
						}
					...
					if (((_n = value.contentBlockStop) == null ? void 0 : _n.contentBlockIndex) != null) {
						...
						if (contentBlock != null) {
							...
							} else if (contentBlock.type === "tool-call") if (usesJsonResponseTool) {
								controller.enqueue({
									type: "text-start",
									id: String(blockIndex)
								});
...
function extractHeaders(headers) {
	let originalHeaders = {};
	if (headers) if (headers instanceof Headers) originalHeaders = convertHeadersToRecord(headers);
	else if (Array.isArray(headers)) for (const [k, v] of headers) originalHeaders[k.toLowerCase()] = v;
	else originalHeaders = Object.fromEntries(Object.entries(headers).map(([k, v]) => [k.toLowerCase(), v]));
...
function createSigV4FetchFunction(getCredentials, fetch$1 = globalThis.fetch) {
	return async (input, init) => {
		var _a;
		if (((_a = init == null ? void 0 : init.method) == null ? void 0 : _a.toUpperCase()) !== "POST" || !(init == null ? void 0 : init.body)) return fetch$1(input, init);
		...
		return fetch$1(input, {
			...init,
			body,
			headers: removeUndefinedEntries(combineHeaders(originalHeaders, signedHeaders))
		});
	};
}
...
function createApiKeyFetchFunction(apiKey, fetch$1 = globalThis.fetch) {
	return async (input, init) => {
		...
		return fetch$1(input, {
			...init,
			headers: removeUndefinedEntries(combineHeaders(originalHeaders, { Authorization: `Bearer ${apiKey}` }))
		});
	};
}
...
export { bedrock, createAmazonBedrock };
