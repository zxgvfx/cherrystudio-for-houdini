import { C as string, M as datetime, O as uuidv4, S as strictObject, _ as object, a as array, c as discriminatedUnion, g as number, i as _null, k as uuidv7, n as _enum, p as literal, w as union } from "./schemas-CV_EtlSZ.js";
import { t as AbsoluteFilePathSchema } from "./file-KsLXrn8b.js";
import { t as GroupIdSchema } from "./group-BHefE3H8.js";
...
const FileItemDataSchema = KnowledgeItemSharedSchema.extend({
	relativePath: string().trim().min(1).describe("Knowledge-base-relative, POSIX-normalized path for the copied source file."),
	indexedRelativePath: string().trim().min(1).optional().describe("Knowledge-base-relative, POSIX-normalized path for the file actually indexed, such as a processed markdown artifact.")
});
const UrlItemDataSchema = KnowledgeItemSharedSchema.extend({
	url: string().trim().min(1).describe("URL to read and index."),
	relativePath: string().trim().min(1).optional().describe("Knowledge-base-relative path for the captured URL snapshot markdown, written on first index.")
});
const NoteItemDataSchema = KnowledgeItemSharedSchema.extend({
	...
	relativePath: string().trim().min(1).optional().describe("Knowledge-base-relative path for the captured note snapshot markdown, written on first index.")
});
const DirectoryItemDataSchema = KnowledgeItemSharedSchema.extend({ relativePath: string().trim().min(1).optional().describe("Knowledge-base-relative `raw/` directory prefix the expanded files are stored under, written on first expansion.") });
union([
	FileItemDataSchema,
	UrlItemDataSchema,
	NoteItemDataSchema,
...
function getKnowledgeItemDisplayTitle(item) {
	...
	switch (item.type) {
		...
		case "note": return (data.relativePath ? getKnowledgePathBasename(data.relativePath).replace(/\.md$/i, "") : "") || getKnowledgeNoteFirstLine(data.content || "");
