import { A as string, C as union, D as unknown, f as _enum, j as any, k as array, r as literal, u as number, v as object, y as record } from "./schemas-DwHFUYcd.js";
...
var OpenRouterProviderMetadataSchema = object({
	...
	usage: object({
		promptTokens: number(),
		promptTokensDetails: object({ cachedTokens: number() }).passthrough().optional(),
		completionTokens: number(),
		completionTokensDetails: object({ reasoningTokens: number() }).passthrough().optional(),
		totalTokens: number(),
		cost: number().optional(),
		costDetails: object({ upstreamInferenceCost: number() }).passthrough()
	}).passthrough()
}).passthrough();
var OpenRouterProviderOptionsSchema = object({ openrouter: object({ reasoning_details: array(ReasoningDetailUnionSchema).optional() }).optional() }).optional();
...
Learn more: https://openrouter.ai/docs/features/multimodal/audio`);
	...
	if (format === "mpeg" || format === "mp3") format = "mp3";
	else if (format === "x-wav" || format === "wave" || format === "wav") format = "wav";
	if (format !== "mp3" && format !== "wav") throw new Error(`Unsupported audio format: "${mediaType}"
			...
			const contentParts = content.map((part) => {
				...
				switch (part.type) {
					...
					case "file": {
						...
						if ((_c2 = part.mediaType) == null ? void 0 : _c2.startsWith("audio/")) return {
							type: "input_audio",
							input_audio: getInputAudioData(part),
							cache_control: cacheControl
						};
						const fileName = String((_g = (_f2 = (_e2 = (_d2 = part.providerOptions) == null ? void 0 : _d2.openrouter) == null ? void 0 : _e2.filename) != null ? _f2 : part.filename) != null ? _g : "");
						const fileData = getFileUrl({
							part,
		...
		case "assistant": {
			...
			for (const part of content) switch (part.type) {
				...
				case "tool-call": {
					...
					if ((partReasoningDetails == null ? void 0 : partReasoningDetails.reasoning_details) && Array.isArray(partReasoningDetails.reasoning_details)) accumulatedReasoningDetails.push(...partReasoningDetails.reasoning_details);
					toolCalls.push({
						id: part.toolCallId,
						type: "function",
						function: {
...
var OpenRouterChatCompletionBaseResponseSchema = object({
	...
	usage: object({
		...
		prompt_tokens_details: object({ cached_tokens: number() }).passthrough().nullish(),
		...
		return {
			...
			providerMetadata: { openrouter: OpenRouterProviderMetadataSchema.parse({
				...
				usage: {
					...
					promptTokensDetails: { cachedTokens: (_s = (_r = (_q = response.usage) == null ? void 0 : _q.prompt_tokens_details) == null ? void 0 : _r.cached_tokens) != null ? _s : 0 },
					completionTokensDetails: { reasoningTokens: (_v = (_u = (_t = response.usage) == null ? void 0 : _t.completion_tokens_details) == null ? void 0 : _u.reasoning_tokens) != null ? _v : 0 },
					costDetails: { upstreamInferenceCost: (_y = (_x = (_w = response.usage) == null ? void 0 : _w.cost_details) == null ? void 0 : _x.upstream_inference_cost) != null ? _y : 0 }
				}
			}) },
			request: { body: args },
			response: {
...
function removeUndefinedEntries2(record$1) {
...
function withUserAgentSuffix(headers, ...userAgentSuffixParts) {
	...
	return __spreadProps(__spreadValues({}, cleanedHeaders), { "user-agent": newUserAgent });
}
var VERSION = "1.2.8";
function createOpenRouter(options = {}) {
	...
	const getHeaders = () => withUserAgentSuffix(__spreadValues({ Authorization: `Bearer ${loadApiKey({
		apiKey: options.apiKey,
		environmentVariableName: "OPENROUTER_API_KEY",
		description: "OpenRouter"
	})}` }, options.headers), `ai-sdk/openrouter/${VERSION}`);
