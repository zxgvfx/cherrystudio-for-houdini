...
import { C as string, _ as object, g as number, n as _enum, s as custom } from "./schemas-CV_EtlSZ.js";
import { n as ipcApi } from "./ipc-BDTAufGC.js";
import { n as FILE_TYPE$1, r as FileTypeSchema, t as AbsoluteFilePathSchema } from "./file-KsLXrn8b.js";
import { g as textExts, l as createFilePathHandle } from "./file-DzPAqnYr.js";
import { n as objectValues } from "./object-C_Vpk_-d.js";
import { i as MB, n as KB, t as GB } from "./constants-6yGJsnWx.js";
...
object({
	...
	purpose: custom((value) => value === void 0 || typeof value === "string").optional()
});
const isImageFileMetadata = (file) => {
	return file.type === FILE_TYPE$1.IMAGE;
};
const FILE_TYPE = {
	IMAGE: "image",
	VIDEO: "video",
...
_enum(objectValues(FILE_TYPE));
const COMPOSER_FILE_KIND = { PASTED_TEXT: "pasted-text" };
const isPastedTextFileMetadata = (file) => {
	return typeof file === "object" && file !== null && file.composerFileKind === COMPOSER_FILE_KIND.PASTED_TEXT;
};
var db_exports = /* @__PURE__ */ __export({ default: () => db_default }, 1);
var db_default;
...
(/* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var extname = require___vite_browser_external().extname;
	...
	function lookup(path) {
		...
		var extension$1 = extname("x." + path).toLowerCase().slice(1);
	...
	function populateMaps(extensions, types) {
		Object.keys(db).forEach(function forEachMimeType(type) {
			...
			for (var i = 0; i < exts.length; i++) {
				...
				types[extension$1] = _preferredType(extension$1, types[extension$1], type);
				const legacyType = _preferredTypeLegacy(extension$1, types[extension$1], type);
				if (legacyType !== types[extension$1]) exports._extensionConflicts.push([
					extension$1,
					legacyType,
					types[extension$1]
				]);
...
function getFileExtension(filePath) {
	...
	if (parts.length > 1) return "." + parts.slice(-1)[0].toLowerCase();
	return ".";
}
...
function formatFileSize(size) {
	if (size >= 1073741824) return (size / GB).toFixed(1) + " GB";
	if (size >= 1048576) return (size / MB).toFixed(1) + " MB";
	if (size >= 1024) return (size / KB).toFixed(0) + " KB";
	return (size / KB).toFixed(2) + " KB";
}
function removeSpecialCharactersForFileName(str) {
	return str.replace(/[<>:"/\\|?*.]/g, "_").replace(/[\r\n]+/g, " ").trim();
}
async function isSupportedFile(filePath, supportExts) {
	try {
		if (supportExts.has(getFileExtension(filePath))) return true;
		const meta = await ipcApi.request("file.get_metadata", createFilePathHandle(AbsoluteFilePathSchema.parse(filePath)));
		if (meta?.kind === "file" && meta.type === "text") return true;
		return false;
	} catch (error) {
		return false;
	}
...
async function isTextFile(filePath) {
	return isSupportedFile(filePath, new Set(textExts));
}
async function filterSupportedFiles(files, supportExts) {
	const extensionSet = new Set(supportExts);
	return (await Promise.all(files.map(async (file) => ({
		file,
		isValid: await isSupportedFile(file.path, extensionSet)
	})))).filter((result) => result.isValid).map((result) => result.file);
}
function parseFileTypes(str) {
	if (Object.values(FILE_TYPE).some((type) => type === str)) return str;
	return null;
}
export { isSupportedFile as a, removeFileExtension as c, FILE_TYPE as d, isPastedTextFileMetadata as f, getFileExtension as i, removeSpecialCharactersForFileName as l, formatFileSize as n, isTextFile as o, isImageFileMetadata as p, getFileDirectory as r, parseFileTypes as s, filterSupportedFiles as t, COMPOSER_FILE_KIND as u };
