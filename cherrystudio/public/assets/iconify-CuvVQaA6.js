...
function addIconSet(storage$1, data) {
	...
	return parseIconSet(data, (name, icon) => {
		if (icon) storage$1.icons[name] = icon;
		else storage$1.missing.add(name);
	});
}
function addIconToStorage(storage$1, name, icon) {
	try {
		if (typeof icon.body === "string") {
			storage$1.icons[name] = { ...icon };
			return true;
		}
	} catch (err) {}
	return false;
...
function addIcon(name, data) {
	...
	if (!icon) return false;
	const storage$1 = getStorage(icon.provider, icon.prefix);
	if (data) return addIconToStorage(storage$1, icon.name, data);
	else {
...
function removeCallback(storages, id) {
	storages.forEach((storage$1) => {
		...
		if (items) storage$1.loaderCallbacks = items.filter((row) => row.id !== id);
	});
}
...
function parseLoaderResponse(storage$1, icons, data) {
	function checkMissing() {
		const pending = storage$1.pendingIcons;
		icons.forEach((name) => {
...
function loadNewIcons(storage$1, icons) {
	if (!storage$1.iconsToLoad) storage$1.iconsToLoad = icons;
	else storage$1.iconsToLoad = storage$1.iconsToLoad.concat(icons).sort();
	if (!storage$1.iconsQueueFlag) {
		storage$1.iconsQueueFlag = true;
		setTimeout(() => {
			storage$1.iconsQueueFlag = false;
			const { provider, prefix } = storage$1;
...
function IconComponent(props) {
	...
	function getInitialState(mounted$1) {
		if (mounted$1) {
			const name$1 = props.icon;
			if (typeof name$1 === "object") return {
				name: "",
				data: name$1
			};
			const data$1 = getIconData(name$1);
			if (data$1) return {
				name: name$1,
				data: data$1
			};
	...
	function updateState() {
		...
		if (changeState({
			...
			if (data$1 === void 0) setAbort({ callback: loadIcons([name$1], updateState) });
			else if (data$1) (_a = props.onLoad) === null || _a === void 0 || _a.call(props, name$1);
		}
	}
