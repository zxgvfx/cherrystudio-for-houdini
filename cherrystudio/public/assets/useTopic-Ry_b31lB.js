...
import { l as useCloseConversationTabs } from "./tab-BoJ166Ld.js";
import { d as toContentRole } from "./message-By01RpZa.js";
import { i as ErrorCode, t as dataApiService } from "./DataApiService-DP44jQXR.js";
import { a as useInvalidateCache, c as useQuery, i as useInfiniteQuery, o as useMutation, r as useInfiniteFlatItems, u as useWriteCache } from "./useDataApi-DhDD9bgI.js";
	...
	try {
		...
		do {
			...
			for (const item of response.items) {
				if (!hasClearContextPart(item.message.data.parts)) pageMessages.push(convertSharedMessage(item.message, assistantId));
				if (item.siblingsGroup) {
					for (const sibling of item.siblingsGroup) if (!hasClearContextPart(sibling.data.parts)) pageMessages.push(convertSharedMessage(sibling, assistantId));
				}
			}
		...
		moveTopic: (0, import_react.useCallback)(async (topicId, { assistantId, anchor }) => {
			...
			try {
				...
				if (assistantChanged) try {
					...
					logger.error("Failed to refresh topics after partial topic move", {
						refreshErr,
						topicId
					});
				}
...
function useActiveTopic({ initialTopic, activeTopicId, setActiveTopicId, passive = false }) {
	const { topic: apiActiveTopic, isLoading: isActiveTopicQueryLoading } = useTopicById(passive || !activeTopicId ? void 0 : activeTopicId);
...
export { useActiveTopic as a, useTopicById as c, startTopicRenaming as i, useTopicMutations as l, getTopicMessages as n, useLatestTopic as o, mapApiTopicToRendererTopic as r, useTopicAutoRenameSync as s, finishTopicRenaming as t, useTopics as u };
