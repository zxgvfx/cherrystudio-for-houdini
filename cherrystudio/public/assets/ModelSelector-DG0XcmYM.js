...
import { c as DynamicVirtualList_default, d as getModelDisplayTags, f as modelMatchesDisplayTag, l as MODEL_DISPLAY_TAGS, m as sortBy, t as isProviderSettingsListVisibleProvider, u as ModelTag } from "./providerSettings-BcvJgtqn.js";
...
import { i as usePortalContainer } from "./portal-container-CZkqeItp.js";
...
import { E as isExternalCliProvider, c as deriveThinkingOptions, n as getSearchMatchScore, u as getModelLogoRef } from "./model-J88ZZ_lQ.js";
import { i as useModels } from "./useModel-CIp4kGrE.js";
import { l as useProviders } from "./useProvider-DEPjZhOC.js";
import { h as isNonChatModel, u as isGatewayRoutableModel } from "./model-D4kd6G9w.js";
					...
					children: [
						search ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								content: multiSelect.ariaLabel ?? multiSelect.label,
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
									...
									className: cn("size-6 shrink-0 rounded-md bg-transparent p-0 shadow-none", multiSelect.checked ? "bg-accent text-accent-foreground" : "text-muted-foreground hover:bg-accent/60 hover:text-foreground"),
...
function useAgentModelFilter(agentType) {
	return (0, import_react.useMemo)(() => {
		const predicate = (model) => {
			if (agentType === "claude-code") return isGatewayRoutableModel(model);
...
function getDuplicateModelNames(models) {
	...
	for (const model of models) nameCounts.set(model.name, (nameCounts.get(model.name) ?? 0) + 1);
...
function useModelSelectorData({ enabled = true, selectedModelIds = [], maxSelectedCount, searchText, filter, showTagFilter = true, showPinnedModels = true, prioritizedProviderIds = [] }) {
	...
	const baseModelFilter = (0, import_react.useCallback)((model) => filter?.(model) ?? true, [filter]);
	...
	const modelsByProvider = (0, import_react.useMemo)(() => {
		...
		for (const model of availableModels) {
			if (!enabledProviderIds.has(model.providerId) || !baseModelFilter(model)) continue;
	...
	const { listItems, modelItems } = (0, import_react.useMemo)(() => {
		...
		const finalModelFilter = (model) => (!showTagFilter || tagFilter(model, providerById.get(model.providerId))) && baseModelFilter(model);
		...
		if (searchText.length === 0 && showPinnedModels && pinnedIdSet.size > 0) {
			const pinnedItems = pinnedIds.flatMap((modelId) => {
				...
				return [createModelItem(model, provider, true, duplicateNamesByProvider.get(provider.id)?.has(model.name) ?? false)];
			});
		...
		sortedProviders.forEach((provider) => {
			...
			items.push(...filteredModels.map((model) => createModelItem(model, provider, showPinnedModels && pinnedIdSet.has(model.id), duplicateNamesByProvider.get(provider.id)?.has(model.name) ?? false)));
		});
		return {
			listItems: items,
...
function ModelSelector(props) {
	...
	const filterContent = (0, import_react.useMemo)(() => {
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			"data-ui": "ui.model-selector.model-selector-filter-tags",
			...
			children: availableTags.map((tag) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ModelTag, {
				tag,
...
export { ModelSelectorRow as a, DEFAULT_SELECTOR_CONTENT_HEIGHT as c, MODEL_SELECTOR_ROW_CHECKBOX_CLASS as i, SelectorShell as l, usePins as n, ModelSelectorRowActionButton as o, useAgentModelFilter as r, getProviderDisplayName as s, ModelSelector as t };
