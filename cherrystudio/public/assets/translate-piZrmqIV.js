...
import { C as string, M as datetime, S as strictObject, _ as object, k as uuidv7, l as emoji, o as boolean, u as int } from "./schemas-CV_EtlSZ.js";
import { c as isAbortError, i as formatErrorMessageWithPrefix } from "./error-3V5V4Mev.js";
import { m as parsePersistedLangCode, p as isTranslateLangCode, s as PersistedLangCodeSchema, w as LANG_DETECT_PROMPT } from "./PreferenceService-uLlqCRc6.js";
import { n as t, t as instance } from "./i18next-DBAWv9hQ.js";
import { t as require_react } from "./react-BgPOU4At.js";
import { t as useTranslation } from "./useTranslation-DnuRkr5k.js";
import { n as usePreference } from "./usePreference-yBX61WcV.js";
import { n as ipcApi } from "./ipc-BDTAufGC.js";
import { c as useQuery, i as useInfiniteQuery, o as useMutation, r as useInfiniteFlatItems } from "./useDataApi-DhDD9bgI.js";
...
import { _ as isQwenMTModel } from "./model-Z1svQByC.js";
import { n as sliceByTokens, t as estimateTokenCount } from "./dist-DY-imOsZ.js";
const UNKNOWN_LANG_CODE = "unknown";
const pickBidirectionalTarget = (sourceLanguageCode, preferred, alter, overrideTarget) => {
	if (overrideTarget) return overrideTarget;
	if (sourceLanguageCode === "unknown") return preferred;
	return sourceLanguageCode === preferred.langCode ? alter : preferred;
};
const getTargetLanguageForBidirectional = (sourceLanguage, languagePair) => {
	if (sourceLanguage === languagePair[0]) return languagePair[1];
	if (sourceLanguage === languagePair[1]) return languagePair[0];
	throw new Error(`Unreachable: sourceLanguage '${sourceLanguage}' is not in pair [${languagePair[0]}, ${languagePair[1]}]`);
};
var isLanguageInPair = (sourceLanguage, languagePair) => {
	return [languagePair[0], languagePair[1]].includes(sourceLanguage);
};
const determineTargetLanguage = (sourceLanguage, targetLanguage, isBidirectional, bidirectionalPair) => {
	if (isBidirectional) {
		if (!isLanguageInPair(sourceLanguage, bidirectionalPair)) return {
			success: false,
...
const handleScrollSync = (sourceElement, targetElement, isProgrammaticScrollRef) => {
	if (isProgrammaticScrollRef.current) return;
	isProgrammaticScrollRef.current = true;
	targetElement.scrollTop = sourceElement.scrollTop / (sourceElement.scrollHeight - sourceElement.clientHeight || 1) * (targetElement.scrollHeight - targetElement.clientHeight || 1);
	requestAnimationFrame(() => {
		isProgrammaticScrollRef.current = false;
	});
};
const createInputScrollHandler = (sourceRef, targetRef, isProgrammaticScrollRef, isScrollSyncEnabled) => {
	return () => {
...
const createOutputScrollHandler = (sourceRef, targetRef, isProgrammaticScrollRef, isScrollSyncEnabled) => {
	return () => {
		const sourceEl = sourceRef.current;
		const targetEl = targetRef.current;
		if (!isScrollSyncEnabled || !sourceEl || !targetEl || isProgrammaticScrollRef.current) return;
...
var TRANSLATE_STREAM_PREFIX = "translate:";
const translateText = async (text, targetLanguage, onResponse, signal) => {
	if (signal?.aborted) throw new DOMException("Translation aborted before start", "AbortError");
	const targetLangCode = typeof targetLanguage === "string" ? targetLanguage : targetLanguage.langCode;
	if (!isTranslateLangCode(targetLangCode) || targetLangCode === "unknown") throw new Error(`Invalid target language: ${targetLangCode}`);
	const streamId = `${TRANSLATE_STREAM_PREFIX}${v4_default()}`;
	let accumulated = "";
	let cleaned = false;
	const unsubscribers = [];
	let abortListener;
	const cleanup = () => {
		if (cleaned) return;
		cleaned = true;
		for (const off of unsubscribers) try {
			off();
		} catch {}
	...
	if (signal) {
		abortListener = () => {
			ipcApi.request("ai.stream.abort", { topicId: streamId }).catch(() => {});
		};
		signal.addEventListener("abort", abortListener, { once: true });
	}
	return new Promise((resolve, reject) => {
		unsubscribers.push(ipcApi.on("ai.stream.chunk", ({ topicId, chunk }) => {
		...
		unsubscribers.push(ipcApi.on("ai.stream.done", ({ topicId }) => {
			if (topicId !== streamId) return;
		...
		unsubscribers.push(ipcApi.on("ai.stream.error", ({ topicId, error }) => {
			if (topicId !== streamId) return;
			cleanup();
...
var parseBuiltinLangCode = (value) => parsePersistedLangCode(value);
const BUILTIN_LANGUAGE = {
	enUS: {
		langCode: parseBuiltinLangCode("en-us"),
		value: "English",
		emoji: "🇺🇸"
	},
	zhCN: {
		langCode: parseBuiltinLangCode("zh-cn"),
		value: "Chinese (Simplified)",
		emoji: "🇨🇳"
	},
	zhTW: {
		langCode: parseBuiltinLangCode("zh-tw"),
		value: "Chinese (Traditional)",
		emoji: "🇭🇰"
	},
	jaJP: {
		langCode: parseBuiltinLangCode("ja-jp"),
		value: "Japanese",
		emoji: "🇯🇵"
	},
	koKR: {
		langCode: parseBuiltinLangCode("ko-kr"),
		value: "Korean",
		emoji: "🇰🇷"
	},
	frFR: {
		langCode: parseBuiltinLangCode("fr-fr"),
		value: "French",
		emoji: "🇫🇷"
	},
	deDE: {
		langCode: parseBuiltinLangCode("de-de"),
		value: "German",
		emoji: "🇩🇪"
	},
	itIT: {
		langCode: parseBuiltinLangCode("it-it"),
		value: "Italian",
		emoji: "🇮🇹"
	},
	esES: {
		langCode: parseBuiltinLangCode("es-es"),
		value: "Spanish",
		emoji: "🇪🇸"
	},
	ptPT: {
		langCode: parseBuiltinLangCode("pt-pt"),
		value: "Portuguese",
		emoji: "🇵🇹"
	},
	ruRU: {
		langCode: parseBuiltinLangCode("ru-ru"),
		value: "Russian",
		emoji: "🇷🇺"
	},
	plPL: {
		langCode: parseBuiltinLangCode("pl-pl"),
		value: "Polish",
		emoji: "🇵🇱"
	},
	arSA: {
		langCode: parseBuiltinLangCode("ar-sa"),
		value: "Arabic",
		emoji: "🇸🇦"
	},
	trTR: {
		langCode: parseBuiltinLangCode("tr-tr"),
		value: "Turkish",
		emoji: "🇹🇷"
	},
	thTH: {
		langCode: parseBuiltinLangCode("th-th"),
		value: "Thai",
		emoji: "🇹🇭"
	},
	viVN: {
		langCode: parseBuiltinLangCode("vi-vn"),
		value: "Vietnamese",
		emoji: "🇻🇳"
	},
	idID: {
		langCode: parseBuiltinLangCode("id-id"),
		value: "Indonesian",
		emoji: "🇮🇩"
	},
	urPK: {
		langCode: parseBuiltinLangCode("ur-pk"),
		value: "Urdu",
		emoji: "🇵🇰"
	},
	msMY: {
		langCode: parseBuiltinLangCode("ms-my"),
		value: "Malay",
		emoji: "🇲🇾"
	},
	ukUA: {
		langCode: parseBuiltinLangCode("uk-ua"),
		value: "Ukrainian",
		emoji: "🇺🇦"
	}
};
const BUILTIN_TRANSLATE_LANGUAGES = Object.values(BUILTIN_LANGUAGE);
const langCodeToI18nKey = new Map(Object.entries({
...
function nGram(n) {
	if (typeof n !== "number" || Number.isNaN(n) || n < 1 || n === Number.POSITIVE_INFINITY) throw new Error("`" + n + "` is not a valid argument for `n-gram`");
	return grams;
	function grams(value) {
		const nGrams = [];
		if (value === null || value === void 0) return nGrams;
...
function collapseWhiteSpace(value, options) {
	if (!options) options = {};
	else if (typeof options === "string") options = { style: options };
...
function asDictionary(value) {
	...
	const dictionary = {};
	let index = -1;
	while (++index < values.length) if (own$1.call(dictionary, values[index])) dictionary[values[index]]++;
	else dictionary[values[index]] = 1;
	return dictionary;
}
function asTuples(value) {
	...
	const tuples = [];
	let trigram$1;
	for (trigram$1 in dictionary) if (own$1.call(dictionary, trigram$1)) tuples.push([trigram$1, dictionary[trigram$1]]);
	tuples.sort(sort$1);
	return tuples;
}
function sort$1(a, b) {
...
var script;
var numericData = {};
for (script in data) if (own.call(data, script)) {
	const languages = data[script];
	let name;
	numericData[script] = {};
	for (name in languages) if (own.call(languages, name)) {
		const model = languages[name].split("|");
		const trigrams$1 = {};
		let weight = model.length;
		while (weight--) trigrams$1[model[weight]] = weight;
		numericData[script][name] = trigrams$1;
	}
}
...
function francAll(value, options = {}) {
	...
	if (!value || value.length < minLength) return und();
	value = value.slice(0, MAX_LENGTH);
	const script$1 = getTopScript(value, expressions);
	if (!script$1[0] || !(script$1[0] in numericData)) {
		if (!script$1[0] || script$1[1] === 0 || !allow(script$1[0], only, ignore)) return und();
		return singleLanguageTuples(script$1[0]);
	}
	return normalize(value, getDistances(asTuples(value), numericData[script$1[0]], only, ignore));
}
function normalize(value, distances) {
	const min = distances[0][1];
	const max = value.length * MAX_DIFFERENCE - min;
	let index = -1;
	while (++index < distances.length) distances[index][1] = 1 - (distances[index][1] - min) / max || 0;
	return distances;
}
function getTopScript(value, scripts) {
	...
	let topScript;
	let script$1;
	for (script$1 in scripts) if (own.call(scripts, script$1)) {
		const count = getOccurrence(value, scripts[script$1]);
		if (count > topCount) {
			topCount = count;
			topScript = script$1;
		}
...
function getDistances(trigrams$1, languages, only, ignore) {
	languages = filterLanguages(languages, only, ignore);
	const distances = [];
	let language;
	if (languages) {
		for (language in languages) if (own.call(languages, language)) distances.push([language, getDistance(trigrams$1, languages[language])]);
	}
	return distances.length === 0 ? und() : distances.sort(sort);
}
function getDistance(trigrams$1, model) {
	let distance = 0;
	let index = -1;
	while (++index < trigrams$1.length) {
		const trigram$1 = trigrams$1[index];
		let difference = MAX_DIFFERENCE;
		if (trigram$1[0] in model) {
			difference = trigram$1[1] - model[trigram$1[0]] - 1;
...
function filterLanguages(languages, only, ignore) {
	if (only.length === 0 && ignore.length === 0) return languages;
	const filteredLanguages = {};
	let language;
	for (language in languages) if (allow(language, only, ignore)) filteredLanguages[language] = languages[language];
	return filteredLanguages;
}
function allow(language, only, ignore) {
	if (only.length === 0 && ignore.length === 0) return true;
...
function useMutationFeedback(mutation, options, context) {
	const { t: t$1 } = useTranslation();
	const mutationRef = (0, import_react.useRef)(mutation);
	...
	return (0, import_react.useCallback)(async (...args) => {
		try {
			const result = await mutationRef.current(...args);
			if (showSuccessToast) toast.success(t$1(contextRef.current.successToastKey));
			return result;
		} catch (e) {
			contextRef.current.logger.error(contextRef.current.errorLogMessage, e);
			if (showErrorToast) toast.error(t$1(contextRef.current.errorToastKey));
...
var logger$4 = loggerService.withContext("translate/useTranslateLanguages");
const useTranslateLanguages = (options) => {
	const { data: data$1, error, refetch } = useQuery("/translate/languages", { enabled: options?.enabled });
	const { t: t$1 } = useTranslation();
	const toastedRef = (0, import_react.useRef)(false);
	(0, import_react.useEffect)(() => {
		if (error && !toastedRef.current) {
			toastedRef.current = true;
			logger$4.error("Failed to load translate languages", error);
			toast.error(t$1("translate.error.languages_load_failed"));
		}
	}, [error, t$1]);
	...
	const getLabel = (0, import_react.useCallback)((lang, withEmoji = true) => {
		if (languages === void 0) return;
		if (isTranslateLangCode(lang)) lang = languages.find((l) => l.langCode === lang) ?? null;
		else if (typeof lang === "string" || lang === null) {
			if (lang !== null) logger$4.warn("getLabel received an invalid lang code, falling back to UNKNOWN", { lang });
			lang = null;
		}
		if (lang === null) {
			const text$1 = t$1("common.unknown");
			return withEmoji ? `🏳️ ${text$1}` : text$1;
		}
		const i18nKey = langCodeToI18nKey.get(lang.langCode);
		const text = i18nKey ? t$1(i18nKey) : lang.value;
		return withEmoji ? `${lang.emoji} ${text}` : text;
	}, [languages, t$1]);
	const getLanguage = (0, import_react.useCallback)((langCode) => {
		if (languages === void 0) return;
		return languages.find((l) => l.langCode === langCode) ?? null;
	}, [languages]);
	const { trigger: addTrigger } = useMutation("POST", "/translate/languages", { refresh: ["/translate/languages"] });
	const { trigger: updateTrigger } = useMutation("PATCH", "/translate/languages/:langCode", { refresh: ["/translate/languages"] });
	const { trigger: removeTrigger } = useMutation("DELETE", "/translate/languages/:langCode", { refresh: ["/translate/languages"] });
	return {
		languages,
		getLabel,
		getLanguage,
		add: useMutationFeedback((0, import_react.useCallback)((data$2) => addTrigger({ body: data$2 }), [addTrigger]), options?.add, {
			logger: logger$4,
		...
		update: useMutationFeedback((0, import_react.useCallback)((langCode, data$2) => {
			if (!langCode) throw new Error("useTranslateLanguages.update: langCode must be set when triggering update");
			return updateTrigger({
				params: { langCode },
				body: data$2
			});
		}, [updateTrigger]), options?.update, {
			logger: logger$4,
		...
		remove: useMutationFeedback((0, import_react.useCallback)((langCode) => {
			if (!langCode) throw new Error("useTranslateLanguages.remove: langCode must be non-empty when triggering delete");
			return removeTrigger({ params: { langCode } });
		}, [removeTrigger]), options?.remove, {
			logger: logger$4,
		...
		error,
		status: languages !== void 0 ? "ready" : error !== void 0 ? "error" : "loading",
		refetch
	};
};
const useLanguages = useTranslateLanguages;
var logger$3 = loggerService.withContext("translate/useDetectLang");
var LLM_INPUT_MAX_TOKENS = 100;
var AUTO_MODE_LLM_THRESHOLD = 100;
const detectLanguageByLLM = async (inputText, langCodes, model) => {
	logger$3.info("Detect language by LLM");
	const text = sliceByTokens(inputText, 0, LLM_INPUT_MAX_TOKENS);
	const listLangText = JSON.stringify(langCodes);
	if (!model) throw new Error(instance.t("error.model.not_exists"));
	if (isQwenMTModel(model)) throw new Error(instance.t("translate.error.detect.qwen_mt"));
	...
	if (!trimmed) throw new Error(instance.t("translate.error.detect.empty"));
	if (!isTranslateLangCode(trimmed)) {
		logger$3.error(`Invalid language code: ${trimmed}`);
		throw new Error(instance.t("translate.error.detect.invalid"));
	}
	return trimmed;
};
const detectLanguageByFranc = (inputText) => {
	logger$3.info("Detect language by franc");
	...
	if (mapped === void 0) {
		logger$3.debug("franc iso3 not in isoMap, falling back to UNKNOWN", { iso3 });
		return UNKNOWN_LANG_CODE;
	}
	return mapped;
};
const detectWithMethod = async (text, method, langCodes, model) => {
	switch (method) {
		case "auto": if (estimateTokenCount(text) < AUTO_MODE_LLM_THRESHOLD) return detectLanguageByLLM(text, langCodes, model);
		else {
			const francResult = detectLanguageByFranc(text);
			if (francResult === "unknown") {
				logger$3.info("franc returned UNKNOWN, falling back to LLM detection");
				return detectLanguageByLLM(text, langCodes, model);
			}
			return francResult;
		}
		case "franc": return detectLanguageByFranc(text);
		case "llm": return detectLanguageByLLM(text, langCodes, model);
		default: throw new Error("Invalid detection method.");
	}
};
const detectLanguageOrUnknown = async (text, detectLanguage, onError) => {
	try {
...
const useDetectLang = () => {
	...
	const toastedEmptyRef = (0, import_react.useRef)(false);
	return (0, import_react.useCallback)(async (inputText) => {
		const text = inputText.trim();
		if (!text) return UNKNOWN_LANG_CODE;
		if (status === "loading") {
			logger$3.warn("useDetectLang invoked while languages were loading, returning UNKNOWN");
			return UNKNOWN_LANG_CODE;
		}
		if (status === "error") {
			logger$3.warn("useDetectLang invoked after languages failed to load, returning UNKNOWN");
			return UNKNOWN_LANG_CODE;
		}
		if (languages === void 0) {
			logger$3.warn("useDetectLang invoked before languages were ready, returning UNKNOWN");
			return UNKNOWN_LANG_CODE;
		}
		if (languages.length === 0) {
			logger$3.error("useDetectLang invoked with an empty language list");
			if (!toastedEmptyRef.current) {
				toastedEmptyRef.current = true;
				toast.error(instance.t("translate.error.languages_load_failed"));
			}
			return UNKNOWN_LANG_CODE;
		}
		const langCodes = languages.map((l) => l.langCode);
		logger$3.info(`Auto detection method: ${method}`);
		const result = await detectWithMethod(text, method, langCodes, quickModel);
		logger$3.info(`Detected language: ${result}`);
		return result;
	}, [
		method,
		languages,
...
var TRANSLATE_ERROR_KEY_PATTERN = /\btranslate\.error\.[a-zA-Z0-9_.-]+\b/;
function localizeTranslateError(error, t$1) {
	if (!(error instanceof Error)) return error;
	const key = error.message.match(TRANSLATE_ERROR_KEY_PATTERN)?.[0];
	if (!key) return error;
	const localizedError = new Error(t$1(key));
...
function useTranslate(options) {
	...
	const activeAbortKeyRef = (0, import_react.useRef)(null);
	const activeControllerRef = (0, import_react.useRef)(null);
	...
	const translate = (0, import_react.useCallback)(async (text, targetLanguage) => {
		activeControllerRef.current?.abort();
		const controller = new AbortController();
		activeControllerRef.current = controller;
		activeAbortKeyRef.current = v4_default();
		const abortKey = activeAbortKeyRef.current;
		setIsTranslating(true);
		const onResponse = optionsRef.current?.onResponse;
		const guardedOnResponse = onResponse ? (chunkText, isComplete) => {
		...
		try {
			...
			if (wasSuperseded() || isAbortError(error)) return;
			...
			loggerService.withContext(opts?.loggerContext ?? "useTranslate").error("Translation failed", error);
			if (showErrorToast) toast.error(formatErrorMessageWithPrefix(localizeTranslateError(error, t$1), t$1(errorPrefixI18nKey)));
...
const TranslateHistorySchema = strictObject({
	id: uuidv7(),
	sourceText: string().min(1),
	targetText: string().min(1),
...
const TranslateLanguageSchema = strictObject({
...
object({
	...
	search: string().min(1).max(200).optional(),
	star: boolean().optional()
}).strict();
TranslateLanguageSchema.pick({
	langCode: true,
	value: true,
	emoji: true
});
...
var logger$2 = loggerService.withContext("translate/useTranslateHistories");
const useTranslateHistories = ({ search, star, pageSize = 20 } = {}) => {
	const searchKey = search?.trim() || void 0;
	const starKey = star || void 0;
	const { pages, error, isLoading, isRefreshing, hasNext, loadNext, refresh: pageRefresh, reset } = useInfiniteQuery("/translate/histories", {
		query: (0, import_react.useMemo)(() => ({
			search: searchKey,
			star: starKey
		}), [searchKey, starKey]),
		limit: pageSize,
	...
	const resetRef = (0, import_react.useRef)(reset);
	...
	(0, import_react.useEffect)(() => {
		if (error && !toastedRef.current) {
			toastedRef.current = true;
			logger$2.error("Failed to load translate histories", error);
			toast.error(t$1("translate.history.error.load"));
		}
	}, [error, t$1]);
	...
	return {
		...
		refresh: (0, import_react.useCallback)(async () => {
			resetRef.current();
			await pageRefresh();
		}, [pageRefresh]),
		status: isLoading ? "loading" : error !== void 0 ? "error" : "ready"
	};
};
var logger$1 = loggerService.withContext("translate/useTranslateHistory");
var toPersistedLangCodeOrNull = (langCode) => {
	if (langCode === null || langCode === void 0 || langCode === "unknown") return null;
	return parsePersistedLangCode(langCode);
};
const useTranslateHistory = (options) => {
	const { trigger: addTrigger } = useMutation("POST", "/translate/histories", { refresh: ["/translate/histories"] });
	const { trigger: updateTrigger } = useMutation("PATCH", "/translate/histories/:id", { refresh: ["/translate/histories"] });
	const { trigger: removeTrigger } = useMutation("DELETE", "/translate/histories/:id", { refresh: ["/translate/histories"] });
	const { trigger: clearTrigger } = useMutation("DELETE", "/translate/histories", { refresh: ["/translate/histories"] });
	return {
		add: useMutationFeedback((0, import_react.useCallback)((data$1) => {
			return addTrigger({ body: {
				sourceText: data$1.sourceText,
				targetText: data$1.targetText,
				sourceLanguage: toPersistedLangCodeOrNull(data$1.sourceLanguage),
				targetLanguage: toPersistedLangCodeOrNull(data$1.targetLanguage)
			} });
		}, [addTrigger]), options?.add, {
			logger: logger$1,
		...
		update: useMutationFeedback((0, import_react.useCallback)((id, data$1) => {
			const body = {};
			if (data$1.sourceText !== void 0) body.sourceText = data$1.sourceText;
			if (data$1.targetText !== void 0) body.targetText = data$1.targetText;
			if ("sourceLanguage" in data$1) body.sourceLanguage = toPersistedLangCodeOrNull(data$1.sourceLanguage);
			if ("targetLanguage" in data$1) body.targetLanguage = toPersistedLangCodeOrNull(data$1.targetLanguage);
			if (data$1.star !== void 0) body.star = data$1.star;
			return updateTrigger({
				params: { id },
				body
			});
		...
		remove: useMutationFeedback((0, import_react.useCallback)((id) => removeTrigger({ params: { id } }), [removeTrigger]), options?.remove, {
			logger: logger$1,
		...
		clear: useMutationFeedback((0, import_react.useCallback)(() => clearTrigger(), [clearTrigger]), options?.clear, {
			logger: logger$1,
...
loggerService.withContext("useTranslateMessage");
export { useDetectLang as a, BUILTIN_LANGUAGE as c, createInputScrollHandler as d, createOutputScrollHandler as f, pickBidirectionalTarget as h, detectLanguageOrUnknown as i, BUILTIN_TRANSLATE_LANGUAGES as l, determineTargetLanguage as m, useTranslateHistories as n, useLanguages as o, UNKNOWN_LANG_CODE as p, useTranslate as r, useTranslateLanguages as s, useTranslateHistory as t, translateText as u };
