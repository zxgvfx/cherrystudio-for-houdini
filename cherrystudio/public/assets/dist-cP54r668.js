import { At as NodeProp, C as indentNodeProp, Dt as tags, Et as styleTags, Ft as Tree, Mt as NodeType, Ot as DefaultBufferLength, Pt as Parser, _ as foldInside, d as continuedIndent, jt as NodeSet, kt as IterMode, o as LanguageSupport, r as LRLanguage, y as foldNodeProp } from "./dist-D5MdU1Xy.js";
var Stack = class Stack {
	...
	reduce(action) {
		...
		let { parser: parser$1 } = this.p;
		let lookaheadRecord = this.reducePos < this.pos - 25 && this.setLookAhead(this.pos);
		let dPrec = parser$1.dynamicPrecedence(type);
		if (dPrec) this.score += dPrec;
		if (depth == 0) {
			this.pushState(parser$1.getGoto(this.state, type, true), this.reducePos);
			if (type < parser$1.minRepeatTerm) this.storeNode(type, this.reducePos, this.reducePos, lookaheadRecord ? 8 : 4, true);
		...
		if (type < parser$1.minRepeatTerm || action & 131072) {
			let pos = parser$1.stateFlag(this.state, 1) ? this.pos : this.reducePos;
			this.storeNode(type, start, pos, count + 4, true);
		}
		if (action & 262144) this.state = this.stack[base];
		else {
			let baseStateID = this.stack[base - 3];
			this.state = parser$1.getGoto(baseStateID, type, true);
		}
	...
	shift(action, type, start, end) {
		if (action & 131072) this.pushState(action & 65535, this.pos);
			...
			let nextState = action, { parser: parser$1 } = this.p;
			this.pos = end;
			let skipped = parser$1.stateFlag(nextState, 1);
			if (!skipped && (end > start || type <= parser$1.maxNode)) this.reducePos = end;
			this.pushState(nextState, skipped ? start : Math.min(start, this.reducePos));
			this.shiftContext(type, start);
			if (type <= parser$1.maxNode) this.buffer.push(type, start, end, 4);
		} else {
			this.pos = end;
			this.shiftContext(type, start);
	...
	recoverByInsert(next) {
		...
		if (nextStates.length > 8 || this.stack.length >= 120) {
			...
			if (this.stack.length < 120) for (let i = 0; best.length < 8 && i < nextStates.length; i += 2) {
				...
				if (!best.some((v, i$1) => i$1 & 1 && v == s)) best.push(nextStates[i], s);
			}
			nextStates = best;
		}
		let result = [];
	...
	forceReduce() {
		let { parser: parser$1 } = this.p;
		let reduce = parser$1.stateSlot(this.state, 5);
		if ((reduce & 65536) == 0) return false;
		if (!parser$1.validAction(this.state, reduce)) {
			let depth = reduce >> 19, term = reduce & 65535;
			let target = this.stack.length - depth * 3;
			if (target < 0 || parser$1.getGoto(this.stack[target], term, false) < 0) {
	...
	findForcedReduction() {
		let { parser: parser$1 } = this.p, seen = [];
		let explore = (state, depth) => {
			if (seen.includes(state)) return;
			seen.push(state);
			return parser$1.allActions(state, (action) => {
				if (action & 393216);
				else if (action & 65536) {
					let rDepth = (action >> 19) - depth;
					if (rDepth > 1) {
						let term = action & 65535, target = this.stack.length - rDepth * 3;
						if (target >= 0 && parser$1.getGoto(this.stack[target], term, false) >= 0) return rDepth << 19 | 65536 | term;
					}
				} else {
	...
	get deadEnd() {
		...
		let { parser: parser$1 } = this.p;
		return parser$1.data[parser$1.stateSlot(this.state, 1)] == 65535 && !parser$1.stateSlot(this.state, 4);
	}
	restart() {
		this.storeNode(0, this.pos, this.pos, 4, true);
...
var TokenGroup = class {
	...
	token(input, stack) {
		let { parser: parser$1 } = stack.p;
		readToken(this.data, input, stack, this.id, parser$1.data, parser$1.tokenPrecTable);
	}
};
...
var TokenCache = class {
	constructor(parser$1, stream) {
		this.stream = stream;
		this.tokens = [];
		this.mainToken = null;
		this.actions = [];
		this.tokens = parser$1.tokenizers.map((_) => new CachedToken());
	}
	getActions(stack) {
		let actionIndex = 0;
		let main = null;
		let { parser: parser$1 } = stack.p, { tokenizers } = parser$1;
		let mask = parser$1.stateSlot(stack.state, 3);
	...
	updateCachedToken(token, tokenizer, stack) {
		...
		if (token.value > -1) {
			let { parser: parser$1 } = stack.p;
			for (let i = 0; i < parser$1.specialized.length; i++) if (parser$1.specialized[i] == token.value) {
				let result = parser$1.specializers[i](this.stream.read(token.start, token.end), stack);
	...
	addActions(stack, token, end, index) {
		let { state } = stack, { parser: parser$1 } = stack.p, { data } = parser$1;
		for (let set = 0; set < 2; set++) for (let i = parser$1.stateSlot(state, set ? 2 : 1);; i += 3) {
...
var Parse = class {
	constructor(parser$1, input, fragments, ranges) {
		...
		this.tokens = new TokenCache(parser$1, this.stream);
		this.topTerm = parser$1.top[1];
		let { from } = ranges[0];
		this.stacks = [Stack.start(this, parser$1.top[0], from)];
		this.fragments = fragments.length && this.stream.end - from > parser$1.bufferLength * 4 ? new FragmentCursor(fragments, parser$1.nodeSet) : null;
	}
	get parsedPos() {
	...
	advanceStack(stack, stacks, split) {
		let start = stack.pos, { parser: parser$1 } = this;
		let base = verbose ? this.stackID(stack) + " -> " : "";
		if (this.stoppedAt != null && start > this.stoppedAt) return stack.forceReduce() ? stack : null;
		if (this.fragments) {
			let strictCx = stack.curContext && stack.curContext.tracker.strict, cxHash = strictCx ? stack.curContext.hash : 0;
			for (let cached = this.fragments.nodeAt(start); cached;) {
				let match = this.parser.nodeSet.types[cached.type.id] == cached.type ? parser$1.getGoto(stack.state, cached.type.id) : -1;
				if (match > -1 && cached.length && (!strictCx || (cached.prop(NodeProp.contextHash) || 0) == cxHash)) {
					stack.useNode(cached, match);
					if (verbose) console.log(base + this.stackID(stack) + ` (via reuse of ${parser$1.getName(cached.type.id)})`);
					return true;
				}
		...
		let defaultReduce = parser$1.stateSlot(stack.state, 4);
		if (defaultReduce > 0) {
			stack.reduce(defaultReduce);
			if (verbose) console.log(base + this.stackID(stack) + ` (via always-reduce ${parser$1.getName(defaultReduce & 65535)})`);
			return true;
		}
		...
		for (let i = 0; i < actions.length;) {
			...
			if (verbose) console.log(base + this.stackID(localStack) + ` (via ${(action & 65536) == 0 ? "shift" : `reduce of ${parser$1.getName(action & 65535)}`} for ${parser$1.getName(term)} @ ${start}${localStack == stack ? "" : ", split"})`);
			if (last) return true;
...
var LRParser = class LRParser extends Parser {
	...
	nextStates(state) {
		...
		for (let i = this.stateSlot(state, 1);; i += 3) {
			...
			if ((this.data[i + 2] & 1) == 0) {
				...
				if (!result.some((v, i$1) => i$1 & 1 && v == value)) result.push(this.data[i], value);
			}
		}
		return result;
	}
	...
	parseDialect(dialect) {
		...
		let disabled = null;
		for (let i = 0; i < values.length; i++) if (!flags[i]) for (let j = this.dialects[values[i]], id$1; (id$1 = this.data[j++]) != 65535;) (disabled || (disabled = new Uint8Array(this.maxTerm + 1)))[id$1] = 1;
...
export { ExternalTokenizer as a, ContextTracker as i, jsonLanguage as n, LRParser as o, jsonParseLinter as r, LocalTokenGroup as s, json as t };
