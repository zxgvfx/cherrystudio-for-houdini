...
import { A as syntaxHighlighting, At as Transaction, Ct as Prec, Dt as StateEffect, E as indentUnit, Ft as findClusterBreak, G as Direction, It as fromCodePoint, J as WidgetType, K as EditorView, L as NodeProp, Mt as codePointSize, Nt as combineConfig, O as matchBrackets, Ot as StateField, P as tags, Pt as countColumn, Q as getPanel, S as getIndentation, T as indentString, Tt as RangeSetBuilder, W as Decoration, X as drawSelection, Y as crosshairCursor, Z as dropCursor, _t as ChangeSet, at as lineNumbers, bt as EditorState, ct as rectangularSelection, et as highlightActiveLine, f as defaultHighlightStyle, ft as crelt, g as foldGutter, gt as ChangeDesc, ht as Annotation, it as keymap, j as syntaxTree, jt as codePointAt, kt as Text, l as bracketMatching, lt as runScopeHandlers, n as IndentContext, nt as highlightSpecialChars, ot as logException, q as ViewPlugin, rt as hoverTooltip, st as placeholder, t as HighlightStyle, tt as highlightActiveLineGutter, ut as showPanel, v as foldKeymap, vt as CharCategory, w as indentOnInput, x as getIndentUnit, xt as Facet, yt as EditorSelection } from "./dist-CoUlMpOM.js";
import { i as closeBracketsKeymap, n as autocompletion, o as completionKeymap, r as closeBrackets } from "./dist-CjXsAxTw.js";
function _extends() {
...
var HistEvent = class HistEvent {
	...
	static fromTransaction(tr, selection$1) {
		...
		return new HistEvent(tr.changes.invert(tr.startState.doc), effects, void 0, selection$1 || tr.startState.selection, none);
	}
...
function isAdjacent(a, b) {
	...
	b.iterChangedRanges((_f, _t, f, t) => {
		for (let i = 0; i < ranges.length;) {
			...
			if (t >= from && f <= to) isAdjacent$1 = true;
		}
	});
	return isAdjacent$1;
}
...
var HistoryState = class HistoryState {
	...
	pop(side, state, onlySelection) {
		...
		let event = branch[branch.length - 1], selection$1 = event.selectionsAfter[0] || (event.startSelection ? event.startSelection.map(event.changes.invertedDesc, 1) : state.selection);
...
var simplifySelection = ({ state, dispatch }) => {
	let cur = state.selection, selection$1 = null;
	if (cur.ranges.length > 1) selection$1 = EditorSelection.create([cur.main]);
	else if (!cur.main.empty) selection$1 = EditorSelection.create([EditorSelection.cursor(cur.main.head)]);
...
function createLineDialog(view) {
	...
	}, crelt$1("label", view.state.phrase("Go to line"), ": ", input), " ", crelt$1("button", {
		class: "cm-button",
		type: "submit"
	}, view.state.phrase("go")), crelt$1("button", {
		name: "close",
		onclick: () => {
			view.dispatch({ effects: dialogEffect.of(false) });
			view.focus();
...
var RegExpQuery = class extends QueryType {
	...
	highlight(state, from, to, add$1) {
		let cursor$1 = regexpCursor(this.spec, state, Math.max(0, from - 250), Math.min(to + 250, state.doc.length));
		while (!cursor$1.next().done) add$1(cursor$1.value.from, cursor$1.value.to);
	}
};
...
var findNext = /* @__PURE__ */ searchCommand((view, { query }) => {
	...
	view.dispatch({
		selection: selection$1,
		effects: [announceMatch(view, next), config.scrollToMatch(selection$1.main, view)],
...
var findPrevious = /* @__PURE__ */ searchCommand((view, { query }) => {
	...
	view.dispatch({
		selection: selection$1,
		effects: [announceMatch(view, prev), config.scrollToMatch(selection$1.main, view)],
		userEvent: "select.search"
	});
	selectSearchInput(view);
	return true;
...
var replaceNext = /* @__PURE__ */ searchCommand((view, { query }) => {
	...
	if (next) {
		selection$1 = EditorSelection.single(next.from, next.to).map(changeSet);
...
var SearchPanel = class {
	constructor(view) {
		...
		this.searchField = crelt$1("input", {
			value: query.search,
		...
		this.replaceField = crelt$1("input", {
			value: query.replace,
		...
		this.dom = crelt$1("div", {
			...
			crelt$1("label", null, [this.caseField, phrase(view, "match case")]),
			crelt$1("label", null, [this.reField, phrase(view, "regexp")]),
			crelt$1("label", null, [this.wordField, phrase(view, "by word")]),
			...view.state.readOnly ? [] : [
				crelt$1("br"),
				this.replaceField,
...
var LintPanel = class LintPanel {
	...
	moveSelection(selectedIndex) {
		...
		let selection$1 = findDiagnostic(this.view.state.field(lintState).diagnostics, this.items[selectedIndex].diagnostic);
...
var basicSetup = function basicSetup$1(options) {
...
var oneDark = [/* @__PURE__ */ EditorView.theme({
	"&": {
		color: ivory,
...
var getDefaultExtensions = function getDefaultExtensions$1(optios) {
...
var ReactCodeMirror = /* @__PURE__ */ (0, import_react.forwardRef)((props, ref) => {
	var { className, value = "", selection: selection$1, extensions = [], onChange, onStatistics, onCreateEditor, onUpdate, autoFocus, theme = "light", height, minHeight, maxHeight, width, minWidth, maxWidth, basicSetup: basicSetup$1, placeholder: placeholder$1, indentWithTab: indentWithTab$1, editable, readOnly, root, initialState } = props, other = _objectWithoutPropertiesLoose(props, _excluded);
	...
	var { state, view, container, setContainer } = useCodeMirror({
		...
		minWidth,
		maxWidth,
		basicSetup: basicSetup$1,
		placeholder: placeholder$1,
		indentWithTab: indentWithTab$1,
		editable,
		readOnly,
		selection: selection$1,
		onChange,
...
export { linter as n, esm_default as t };
