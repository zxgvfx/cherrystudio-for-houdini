...
import { n as parseFontSize } from "./chunk-U37J5Y7L-UQ_9cfTv.js";
import { $ as darken_default, F as log, M as sanitizeText, W as setupGraphViewbox, a1 as lighten_default, a2 as is_dark_default, d as __name, n as defaultConfig_default, w as getConfig2 } from "./src-XfX5FL_o.js";
import "./dist-B4A8UMUm.js";
import "./math-Cgxj31JI.js";
import { c as createText } from "./chunk-62K37W7T-Cdcrszm7.js";
	...
	var lexer = /* @__PURE__ */ function() {
		var lexer2 = {
			...
			upcomingInput: /* @__PURE__ */ __name(function() {
				var next$1 = this.match;
				if (next$1.length < 20) next$1 += this._input.substr(0, 20 - next$1.length);
				return (next$1.substr(0, 20) + (next$1.length > 20 ? "..." : "")).replace(/\n/g, "");
			}, "upcomingInput"),
...
var MindmapDB = class {
	...
	addNode(level, id, descr, type) {
		...
		} else if (this.nodes.length === 0) this.nodes.push(node);
...
var defaultBkg = /* @__PURE__ */ __name(function(db, elem, node, section) {
	const rd = 5;
	elem.append("path").attr("id", "node-" + node.id).attr("class", "node-bkg node-" + db.type2Str(node.type)).attr("d", `M0 ${node.height - rd} v${-node.height + 2 * rd} q0,-5 5,-5 h${node.width - 2 * rd} q5,0 5,5 v${node.height - rd} H0 Z`);
...
var cloudBkg = /* @__PURE__ */ __name(function(db, elem, node) {
	...
	const r3 = .35 * w;
	const r4 = .2 * w;
	elem.append("path").attr("id", "node-" + node.id).attr("class", "node-bkg node-" + db.type2Str(node.type)).attr("d", `M0 0 a${r1},${r1} 0 0,1 ${w * .25},${-1 * w * .1}
...
var bangBkg = /* @__PURE__ */ __name(function(db, elem, node) {
	...
	const h = node.height;
	const r = .15 * w;
	elem.append("path").attr("id", "node-" + node.id).attr("class", "node-bkg node-" + db.type2Str(node.type)).attr("d", `M0 0 a${r},${r} 1 0,0 ${w * .25},${-1 * h * .1}
      a${r},${r} 1 0,0 ${w * .25},0
      a${r},${r} 1 0,0 ${w * .25},0
...
var drawNode = /* @__PURE__ */ __name(async function(db, elem, node, fullSection, conf) {
	...
	if (node.icon) if (node.type === db.nodeType.CIRCLE) {
		node.height += 50;
		node.width += 50;
...
async function drawNodes(db, svg, mindmap, section, conf) {
	...
	if (mindmap.children) await Promise.all(mindmap.children.map((child, index) => drawNodes(db, svg, child, section < 0 ? index : section, conf)));
}
...
var mindmapRenderer_default = { draw };
