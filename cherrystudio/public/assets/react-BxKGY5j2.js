import { c as __toESM, n as __esmMin, o as __toCommonJS, r as __export } from "./rolldown-runtime-D8OvLAZx.js";
...
var isEasingArray = /* @__NO_SIDE_EFFECTS__ */ (ease$1) => {
	...
	const generator = {
		...
		next: (t) => {
			if (!isResolvedFromDuration && dampingRatio < 1) {
				...
				const current$1 = target - envelope * (A * sin + initialDelta * cos);
...
function keyframes({ duration = 300, keyframes: keyframeValues, times, ease: ease$1 = "easeInOut" }) {
	const easingFunctions = /* @__PURE__ */ isEasingArray(ease$1) ? ease$1.map(easingDefinitionToFunction) : easingDefinitionToFunction(ease$1);
	const state = {
		done: false,
...
var NativeAnimationExtended = class extends NativeAnimation {
	...
	updateMotionValue(value) {
		...
		motionValue$1.setWithVelocity(sampleAnimation.sample(Math.max(0, sampleTime - delta)).value, current, delta);
...
function animateTarget(visualElement, targetAndTransition, { delay: delay$1 = 0, transitionOverride, type } = {}) {
	...
	if (path) path.animateVisualElement(visualElement, target, transition, delay$1, animations$1);
...
function buildProjectionTransform(delta, treeScale, latestTransform) {
	...
	if (latestTransform) {
		const { transformPerspective, rotate: rotate$1, pathRotation, rotateX, rotateY, skewX, skewY } = latestTransform;
...
function createProjectionNode$1({ attachResizeListener, defaultParent, measureScroll, checkIsScrollRoot, resetTransform }) {
	return class ProjectionNode {
		...
		setAnimationOrigin(delta, hasOnlyRelativeTargetChanged = false, pathFn) {
			...
			const interpolate$1 = pathFn?.interpolateProjection(delta);
			this.mixTargetDelta = (latest) => {
				const progress$1 = latest / 1e3;
				const point = interpolate$1?.(progress$1);
				if (point) {
					targetDelta.x.translate = point.x;
					targetDelta.x.scale = mixNumber(delta.x.scale, 1, progress$1);
					targetDelta.x.origin = delta.x.origin;
					targetDelta.x.originPoint = delta.x.originPoint;
					targetDelta.y.translate = point.y;
					targetDelta.y.scale = mixNumber(delta.y.scale, 1, progress$1);
					targetDelta.y.origin = delta.y.origin;
					targetDelta.y.originPoint = delta.y.originPoint;
				} else {
					mixAxisDeltaLinear(targetDelta.x, delta.x, progress$1);
					mixAxisDeltaLinear(targetDelta.y, delta.y, progress$1);
				}
...
var m = m$1;
export { PresenceContext as _, createDomVisualElement as a, LayoutGroupContext as b, usePresence as c, hasReducedMotionListener as d, prefersReducedMotion as f, invariant as g, setTarget as h, animations as i, MotionConfigContext as l, animateVisualElement as m, motion as n, loadFeatures as o, isHTMLElement as p, gestureAnimations as r, LazyContext as s, m as t, initPrefersReducedMotion as u, useIsomorphicLayoutEffect as v, useConstant as y };
