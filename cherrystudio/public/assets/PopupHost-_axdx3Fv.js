...
import { n as getCmThemeNames, t as getCmThemeByName } from "./utils-DdeIoKIl.js";
import { t as CodeStyleContext } from "./useCodeStyle-ZK5esKOR.js";
import { n as useTheme } from "./useTheme-C0NcZaKl.js";
import { t as useMermaid } from "./useMermaid-CRrUmNsD.js";
import { c as loadThemeIfNeeded, i as getMarkdownIt, n as DEFAULT_THEMES, o as getShiki, r as getHighlighter, s as loadLanguageIfNeeded, t as DEFAULT_LANGUAGES } from "./shiki-408EkuKH.js";
...
import { r as popupService } from "./popup-C0FVl5N5.js";
...
var ShikiStreamService = class {
	...
	async initWorker() {
		...
		this.workerInitPromise = (async () => {
			try {
				this.worker = new (await (__vitePreload(() => import("./shikiStream.worker-xBtoaHxn.js"), [], import.meta.url))).default();
...
const CodeStyleProvider = ({ children }) => {
	...
	(0, import_react.useEffect)(() => {
		if (codeEditorEnabled) getCmThemeNames().then(setCmThemeNames);
		else getShiki().then(({ bundledThemesInfo }) => {
	...
	const activeShikiTheme = (0, import_react.useMemo)(() => {
		...
		if (!codeStyle || codeStyle === "auto" || !themeNames.includes(codeStyle)) return theme === ThemeMode.light ? "one-light" : "material-theme-darker";
		return codeStyle;
	}, [
		theme,
		codeViewerThemeLight,
		codeViewerThemeDark,
		themeNames
	]);
	const isShikiThemeDark = (0, import_react.useMemo)(() => {
		return shikiThemesInfo.find((info) => info.id === activeShikiTheme)?.type === "dark";
	...
	(0, import_react.useEffect)(() => {
		...
		if (!themeName || themeName === "auto" || !themeNames.includes(themeName)) themeName = theme === ThemeMode.light ? "materialLight" : "dark";
		let cancelled = false;
