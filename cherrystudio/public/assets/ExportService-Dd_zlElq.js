import { n as __esmMin, o as __toCommonJS, r as __export, s as __toESM, t as __commonJSMin } from "./chunk-DiqNceaa.js";
import { t as loggerService } from "./LoggerService-oVV4iwe6.js";
import { t as preferenceService } from "./PreferenceService-uLlqCRc6.js";
import { c as getNamingTextContent, l as getThinkingContent, n as fetchMessagesSummary, o as getCitationContent, s as getMainTextContent, u as getToolCitationExport } from "./aiGeneration-Dqw73yb1.js";
import { t as require_dayjs_min } from "./dayjs.min-CNu3tPBh.js";
import { r as resolver_default } from "./resolver-eUld2ti5.js";
import { n as require_extend } from "./lib-BGOUa2xQ.js";
import { A as markdownToPlainText, D as convertMathFormula, r as stripCitationMarkers } from "./citations-CtEFOa63.js";
...
import { i as captureScrollableAsDataUrl, r as captureScrollableAsBlob } from "./image-Bb803VSe.js";
import { n as getTopicMessages } from "./useTopic-Ry_b31lB.js";
import { l as removeSpecialCharactersForFileName } from "./file-AcqqP-OX.js";
import { t as getComposerTextFromMessage } from "./composerTokens-CFq729cv.js";
import { t as require___vite_browser_external } from "./__vite-browser-external-BDenMFlt.js";
import { t as require_src$1 } from "./src-CCT6_PBr.js";
import { r as addNote } from "./useNotesSettings-Cdvj7OQW.js";
...
const processCitations = (content$3, mode = "remove") => {
	return content$3.split(/(```[a-zA-Z]*\n[\s\S]*?\n```)/g).map((part, index$1) => {
		if (index$1 % 2 === 1) return part;
		let result = part;
...
var formatMessageAsPlainText = (message$1) => {
	const author = "messageSnapshot" in message$1 ? message$1.messageSnapshot : void 0;
	return `${message$1.role === "user" ? "User:" : `${author?.name ?? "Assistant"}:`}\n${markdownToPlainText(copyableTextContent(message$1)).trim()}`;
};
var copyableTextContent = (message$1) => {
	return getToolCitationExport(message$1, getComposerTextFromMessage(message$1, getNamingTextContent(message$1))).content;
};
const messageToPlainText = (message$1) => {
	return markdownToPlainText(copyableTextContent(message$1)).trim();
};
...
var require_src = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function markdownToBlocks$1(body, options) {
	...
	exports.markdownToBlocks = markdownToBlocks$1;
...
[DEBUG] appendInternal called with ${children.length} children for block ${block_id}`);
				...
				for (let chunk of chunks) {
					...
					if (response) {
						...
						for (let [index$1, block2] of chunk.entries()) if (chunkChildren[index$1].length > 0) {
							if (results[index$1] && block2.type === results[index$1].type) {
								const nestedResponses = await appendInternal({
									block_id: results[index$1].id,
									children: chunkChildren[index$1],
									client,
									apiCall,
									getResults,
									debug
...
var sanitizeReasoningContent = (content$3) => {
	const contentWithBr = content$3.replace(/\n/g, "<br>");
...
var formatCitationsAsFootnotes = (citations) => {
	...
	return citations.split("\n\n").map((line) => {
		...
		if (match) {
			const [, num, content$3] = match;
			return `[^${num}]: ${content$3}`;
		}
		return line;
	}).join("\n\n");
};
var createBaseMarkdown = async (message$1, includeReasoning = false, excludeCitations = false, normalizeCitations = true) => {
	const forceDollarMathInMarkdown = await preferenceService.get("data.export.markdown.force_dollar_math");
	const author = "messageSnapshot" in message$1 ? message$1.messageSnapshot : void 0;
	const model = message$1.model ?? author?.model;
	const titleSection = `## ${await getRoleText(message$1.role, model?.name, model?.provider, author)}`;
	let reasoningSection = "";
	if (includeReasoning) {
		let reasoningContent = getThinkingContent(message$1);
		if (reasoningContent) {
...
`;
	...
	const { content: content$3, citation: toolCitation } = getToolCitationExport(message$1, getComposerTextFromMessage(message$1, getMainTextContent(message$1)));
	let citation = excludeCitations ? "" : getCitationContent(message$1) || toolCitation;
	let processedContent = forceDollarMathInMarkdown ? convertMathFormula(content$3) : content$3;
...
async function getMessageTitle(message$1, length = 30) {
	const content$3 = getNamingTextContent(message$1);
	if (await preferenceService.get("data.export.markdown.use_topic_naming_for_message_title")) try {
		const titlePromise = fetchMessagesSummary({ messages: [message$1] });
		toast.loading({
	...
	let title = getTitleFromString(content$3, length);
	if (!title) title = (0, import_dayjs_min.default)(message$1.createdAt).format("YYYYMMDDHHmm");
	return title;
}
const messageToMarkdown = async (message$1, excludeCitations) => {
	...
	const { titleSection, contentSection, citation } = await createBaseMarkdown(message$1, false, excludeCitations ?? excludeCitationsInExport, standardizeCitationsInExport);
	return [
		titleSection,
		"",
		contentSection,
...
const messageToMarkdownWithReasoning = async (message$1, excludeCitations) => {
	const { excludeCitationsInExport, standardizeCitationsInExport } = await preferenceService.getMultiple({
		excludeCitationsInExport: "data.export.markdown.exclude_citations",
		standardizeCitationsInExport: "data.export.markdown.standardize_citations"
	});
	const { titleSection, reasoningSection, contentSection, citation } = await createBaseMarkdown(message$1, true, excludeCitations ?? excludeCitationsInExport, standardizeCitationsInExport);
	return [
		titleSection,
		"",
		reasoningSection,
...
const messagesToMarkdown = async (messages, exportReasoning, excludeCitations) => {
	...
	return (await Promise.all(messages.map((message$1) => converter(message$1, excludeCitations)))).join("\n---\n");
};
...
const exportMarkdownContentAsFile = async (title, markdown$1) => {
	if (getExportState()) {
	...
	if (!markdownExportPath) try {
		...
		if (await window.api.file.save(fileName, markdown$1)) toast.success(resolver_default.t("message.success.markdown.export.specified"));
	} catch (error) {
		...
		await window.api.file.write(markdownExportPath + "/" + fileName, markdown$1);
...
const exportTopicAsMarkdown = async (topic, exportReasoning, excludeCitations) => {
	...
	if (!markdownExportPath) try {
		...
		const markdown$1 = await topicToMarkdown(topic, exportReasoning, excludeCitations);
		if (await window.api.file.save(fileName, markdown$1)) toast.success(resolver_default.t("message.success.markdown.export.specified"));
	} catch (error) {
		toast.error(resolver_default.t("message.error.markdown.export.specified"));
...
const exportMessageAsMarkdown = async (message$1, exportReasoning, excludeCitations) => {
	if (getExportState()) {
		toast.warning(resolver_default.t("message.warn.export.exporting"));
		return;
	}
	...
	const markdownExportPath = await preferenceService.get("data.export.markdown.path");
	if (!markdownExportPath) try {
		const fileName = removeSpecialCharactersForFileName(await getMessageTitle(message$1)) + ".md";
		const markdown$1 = exportReasoning ? await messageToMarkdownWithReasoning(message$1, excludeCitations) : await messageToMarkdown(message$1, excludeCitations);
		if (await window.api.file.save(fileName, markdown$1)) toast.success(resolver_default.t("message.success.markdown.export.specified"));
	} catch (error) {
		toast.error(resolver_default.t("message.error.markdown.export.specified"));
		...
		const timestamp = (0, import_dayjs_min.default)().format("YYYY-MM-DD-HH-mm-ss");
		const fileName = removeSpecialCharactersForFileName(await getMessageTitle(message$1)) + ` ${timestamp}.md`;
		const markdown$1 = exportReasoning ? await messageToMarkdownWithReasoning(message$1, excludeCitations) : await messageToMarkdown(message$1, excludeCitations);
		await window.api.file.write(markdownExportPath + "/" + fileName, markdown$1);
		toast.success(resolver_default.t("message.success.markdown.export.preconf"));
	} catch (error) {
...
var convertMarkdownToNotionBlocks = async (markdown$1) => {
	return (0, import_src$1.markdownToBlocks)(markdown$1);
};
var convertThinkingToNotionBlocks = async (thinkingContent) => {
	if (!thinkingContent.trim()) return [];
	try {
		const childrenBlocks = (0, import_src$1.markdownToBlocks)(thinkingContent.replace(/<br\s*\/?>/g, "\n"));
		return [{
			object: "block",
			type: "toggle",
			toggle: {
...
var executeNotionExport = async (title, allBlocks) => {
	...
	try {
		const notion$1 = new import_src.Client({ auth: notionApiKey });
		const responsePromise = notion$1.pages.create({
...
const exportMessageToNotion = async (title, content$3, message$1) => {
	const notionExportReasoning = await preferenceService.get("data.integration.notion.export_reasoning");
	const notionBlocks = await convertMarkdownToNotionBlocks(content$3);
	if (notionExportReasoning && message$1) {
		const thinkingContent = stripCitationMarkers(getThinkingContent(message$1));
		if (thinkingContent) {
...
const exportMessagesToNotion = async (title, messages) => {
	...
	for (const message$1 of messages) {
		const messageBlocks = await convertMarkdownToNotionBlocks(await messageToMarkdown(message$1, excludeCitationsInExport));
...
const exportMarkdownToYuque = async (title, content$3) => {
...
const exportMarkdownToJoplin = async (title, contentOrMessages) => {
	...
	setExportingState(true);
	let content$3;
	if (typeof contentOrMessages === "string") content$3 = contentOrMessages;
	else if (Array.isArray(contentOrMessages)) content$3 = await messagesToMarkdown(contentOrMessages, joplinExportReasoning, excludeCitationsInExport);
	else content$3 = joplinExportReasoning ? await messageToMarkdownWithReasoning(contentOrMessages, excludeCitationsInExport) : await messageToMarkdown(contentOrMessages, excludeCitationsInExport);
	try {
...
const exportMarkdownToSiyuan = async (title, content$3) => {
	...
	try {
		...
		await createSiyuanDoc(siyuanApiUrl, siyuanToken, siyuanBoxId, `${await renderSprigTemplate(siyuanApiUrl, siyuanToken, siyuanRootPath?.startsWith("/") ? siyuanRootPath : `/${siyuanRootPath || "CherryStudio"}`)}/${`${title.replace(/[#|\\^\\[\]]/g, "")}`}`, content$3);
...
async function createSiyuanDoc(apiUrl, token, boxId, path$1, markdown$1) {
...
var saveContentToNotes = async (title, content$3, folderPath) => {
	await addNote(title, content$3, folderPath);
...
const exportContentToNotes = async (title, content$3, folderPath) => {
	try {
		await saveContentToNotes(title, content$3, folderPath);
	} catch (error) {
		handleNotesExportError(error);
		throw error;
	}
};
const exportMessageToNotes = async (title, content$3, folderPath) => {
	await exportContentToNotes(title, content$3.replace(/^## 🤖 Assistant(\n|$)/m, ""), folderPath);
};
const exportTopicToNotes = async (topic, folderPath) => {
	try {
		const content$3 = await topicToMarkdown(topic);
		await saveContentToNotes(topic.name, content$3, folderPath);
	} catch (error) {
		handleNotesExportError(error);
		throw error;
	}
};
var exportNoteAsMarkdown = async (noteName, content$3) => {
	const markdown$1 = `# ${noteName}\n\n${content$3}`;
...
const exportNote = async ({ node: node$1, platform }) => {
	try {
		const content$3 = await window.api.file.readExternal(node$1.externalPath);
		switch (platform) {
			case "copyImage": return await exportNoteAsImageToClipboard();
			case "exportImage": return await exportNoteAsImageFile(node$1.name);
			case "markdown": return await exportNoteAsMarkdown(node$1.name, content$3);
			case "docx":
				ipcApi.request("export.word.from_markdown", {
					markdown: `# ${node$1.name}\n\n${content$3}`,
					fileName: removeSpecialCharactersForFileName(node$1.name)
				});
				return;
			case "notion":
				await exportMessageToNotion(node$1.name, content$3);
				return;
			case "yuque":
				await exportMarkdownToYuque(node$1.name, `# ${node$1.name}\n\n${content$3}`);
				return;
			case "joplin":
				await exportMarkdownToJoplin(node$1.name, content$3);
				return;
			case "siyuan":
				await exportMarkdownToSiyuan(node$1.name, `# ${node$1.name}\n\n${content$3}`);
				return;
		}
	} catch (error) {
...
export { messagesToPlainText as S, messageToMarkdownWithReasoning as _, exportMarkdownToSiyuan as a, topicToPlainText as b, exportMessageToNotes as c, exportNote as d, exportTopicAsMarkdown as f, messageToMarkdown as g, getMessageTitle as h, exportMarkdownToObsidian as i, exportMessageToNotion as l, exportTopicToNotion as m, exportMarkdownContentAsFile as n, exportMarkdownToYuque as o, exportTopicToNotes as p, exportMarkdownToJoplin as r, exportMessageAsMarkdown as s, exportContentToNotes as t, exportMessagesToNotion as u, messagesToMarkdown as v, messageToPlainText as x, topicToMarkdown as y };
