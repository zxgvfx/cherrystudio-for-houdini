...
function dataApiRequestTimeoutMs() {
	return isDccPanelAttach() ? 12e3 : 3e3;
}
var DataApiService = class {
	...
	defaultRetryOptions = {
		...
		retryDelay: isDccPanelAttach() ? 1500 : 1e3,
	...
	async sendRequest(request, retryCount = 0) {
		...
		try {
			...
			const response = await Promise.race([window.api.dataApi.request(request), new Promise((_, reject) => setTimeout(() => reject(DataApiErrorFactory.timeout(request.path, timeoutMs, requestContext)), timeoutMs))]);
			if (response.error) {
