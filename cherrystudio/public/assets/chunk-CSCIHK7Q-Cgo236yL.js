...
import { i as setLogLevel, n as __name, r as log, t as __export } from "./src-DnWfLfuG.js";
var Channel = {
	min: {
		r: 0,
		g: 0,
	...
	for (const key of Object.keys(args)) {
		...
		if (typeof args[key] === "object") {
			if (key === "nodeColors") {
				...
				for (const colorKey of Object.keys(args[key])) if (typeof args[key][colorKey] !== "string" || !colorPattern.test(args[key][colorKey])) {
...
var renderKatexUnsanitized = /* @__PURE__ */ __name(async (text, config2) => {
	...
	if (!(isMathMLSupported() || config2.legacyMathML || config2.forceLegacyMathML)) return text.replace(katexRegex, "MathML is unsupported in this environment.");
	{
		const { default: katex } = await __vitePreload(async () => {
			const { default: katex$1 } = await import("./katex-BJUe9V6g.js");
