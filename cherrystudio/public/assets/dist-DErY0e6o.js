import { n as keyName, r as shift, t as base } from "./w3c-keyname-d7Yk5myI.js";
...
var TextLeaf = class TextLeaf extends Text {
	...
	lineInner(target, isLine, line, offset) {
		for (let i = 0;; i++) {
			let string$1 = this.text[i], end = offset + string$1.length;
...
var ChangeSet = class ChangeSet extends ChangeDesc {
	...
	static fromJSON(json) {
		...
		for (let i = 0; i < json.length; i++) {
			...
			else if (!Array.isArray(part) || typeof part[0] != "number" || part.some((e, i$1) => i$1 && typeof e != "string")) throw new RangeError("Invalid JSON representation of ChangeSet");
...
function compareArray(a, b, compare$1) {
	...
	for (let i = 0; i < a.length; i++) if (!compare$1(a[i], b[i])) return false;
	return true;
}
...
var EditorState = class EditorState {
	...
	phrase(phrase, ...insert$1) {
		...
		if (insert$1.length) phrase = phrase.replace(/\$(\$|\d*)/g, (m, i) => {
			if (i == "$") return "$";
			let n = +(i || 1);
...
var RangeSet = class RangeSet {
	...
	update(updateSpec) {
		...
		while (cur.value || i < add$1.length) if (i < add$1.length && (cur.from - add$1[i].from || cur.startSide - add$1[i].value.startSide) >= 0) {
			let range = add$1[i++];
			if (!builder.addInner(range.from, range.to, range.value)) spill.push(range);
		} else if (cur.rangeIndex == 1 && cur.chunkIndex < this.chunk.length && (i == add$1.length || this.chunkEnd(cur.chunkIndex) < add$1[i].from) && (!filter || filterFrom > this.chunkEnd(cur.chunkIndex) || filterTo < this.chunkPos[cur.chunkIndex]) && builder.addChunk(this.chunkPos[cur.chunkIndex], this.chunk[cur.chunkIndex])) cur.nextChunk();
		else {
	...
	static join(sets) {
		...
		for (let i = sets.length - 2; i >= 0; i--) for (let layer$1 = sets[i]; layer$1 != RangeSet.empty; layer$1 = layer$1.nextLayer) result = new RangeSet(layer$1.chunkPos, layer$1.chunk, result, Math.max(layer$1.maxPoint, result.maxPoint));
		return result;
	}
};
...
function countColumn(string$1, tabSize, to = string$1.length) {
	let n = 0;
	for (let i = 0; i < to && i < string$1.length;) if (string$1.charCodeAt(i) == 9) {
		n += tabSize - n % tabSize;
		i++;
	} else {
		n++;
		i = findClusterBreak(string$1, i);
	}
	return n;
}
function findColumn(string$1, col, tabSize, strict) {
	for (let i = 0, n = 0;;) {
		if (n >= col) return i;
		if (i == string$1.length) break;
		n += string$1.charCodeAt(i) == 9 ? tabSize - n % tabSize : 1;
		i = findClusterBreak(string$1, i);
	}
...
function setAttrs(dom, attrs) {
	for (let i = dom.attributes.length - 1; i >= 0; i--) {
		let name$1 = dom.attributes[i].name;
...
function scrollableParents(dom, getX = true) {
	let doc$1 = dom.ownerDocument, x = null, y = null;
	for (let cur = dom.parentNode; cur;) if (cur == doc$1.body || (!getX || x) && y) break;
...
function focusPreventScroll(dom) {
	...
	if (!preventScrollSupported) {
		...
		for (let i = 0; i < stack.length;) {
			let elt = stack[i++], top$1 = stack[i++], left = stack[i++];
...
function dispatchKey(elt, name$1, code, mods) {
	let options = {
		key: name$1,
		code: name$1,
		keyCode: code,
	...
	for (let iI = 0; iI <= isolates.length; iI++) {
		...
		for (let i = from, prev = prevType, prevStrong = prevType; i < to; i++) {
			...
			if (type == 128) if (i < to - 1 && prev == types[i + 1] && prev & 24) type = types[i] = prev;
				...
				let replace$1 = i && prev == 8 || end < rTo && types[end] == 8 ? prevStrong == 1 ? 1 : 8 : 256;
				for (let j = i; j < end; j++) types[j] = replace$1;
				i = end - 1;
	...
	for (let iI = 0, sI = 0, context = 0; iI <= isolates.length; iI++) {
		...
		for (let i = from, ch, br, type; i < to; i++) if (br = Brackets[ch = line.charCodeAt(i)]) if (br < 0) {
			for (let sJ = sI - 3; sJ >= 0; sJ -= 3) if (BracketStack[sJ + 1] == -br) {
				...
				if (type$1) types[i] = types[BracketStack[sJ]] = type$1;
				sI = sJ;
				break;
			}
...
var scrollIntoView = /* @__PURE__ */ StateEffect.define({ map: (t$1, ch) => t$1.map(ch) });
...
var LineTile = class LineTile extends CompositeTile {
	...
	resolveInline(pos, side, forCoords) {
		...
		function scan(tile, pos$1) {
			for (let i = 0, off = 0; i < tile.children.length && off <= pos$1; i++) {
...
var TileBuilder = class {
	...
	ensureMarks(marks$1, openStart) {
		...
		for (let i = marks$1.length - 1; i >= 0; i--) {
			let mark = marks$1[i], last;
...
var TileUpdate = class {
			...
			span: (from$1, to$1, active, openStart) => {
				for (let pos = from$1; pos < to$1;) {
					...
					if (chars == null) {
						...
						b.ensureLine(pendingLineAttrs);
						b.addText(chars, active, pos == from$1 ? openStart : active.length);
						pos += chars.length;
					}
					pendingLineAttrs = null;
				}
	...
	getCompositionContext(text) {
		...
		for (let parent = text.parentNode;; parent = parent.parentNode) {
			...
			else if (!line) marks$1.push(MarkTile.of(new MarkDecoration({
...
var DocView = class {
	...
	updateDeco() {
		...
		let dynamicOuter = false, outerDeco = this.view.state.facet(outerDecorations).map((d, i$1) => {
...
var InlineCoordsScan = class {
	...
	scan(positions, getRects) {
		...
		search: while (lo < hi) {
			let dist$1 = hi - lo, mid = lo + hi >> 1;
			adjust: if (seen.has(mid)) {
				let scan = lo + Math.floor(Math.random() * dist$1);
	...
	scanTile(tile, offset) {
		...
		if (tile.children.length == 1) {
			let child$1 = tile.children[0];
			if (child$1.isText()) return this.scanText(child$1, offset);
			else if (child$1.isComposite()) return this.scanTile(child$1, offset);
		}
		let positions = [offset];
		for (let i = 0, pos$1 = offset; i < tile.children.length; i++) positions.push(pos$1 += tile.children[i].length);
		let scan = this.scan(positions, (i) => {
			let child$1 = tile.children[i];
			if (child$1.flags & 48) return null;
			return (child$1.dom.nodeType == 1 ? child$1.dom : textRange(child$1.dom, 0, child$1.length)).getClientRects();
		});
...
function doPaste(view, input) {
	...
	if (lastLinewiseCopy != null && state.selection.ranges.every((r) => r.empty) && lastLinewiseCopy == text.toString()) {
		...
		changes = state.changeByRange((range) => {
			...
			lastLine = line.from;
			let insert$1 = state.toText((byLine ? text.line(i++).text : input) + state.lineBreak);
			return {
				changes: {
					from: line.from,
					insert: insert$1
...
var HeightMapBlock = class extends HeightMap {
	...
	forEachLine(from, to, oracle, top$1, offset, f) {
		if (from <= offset + this.length && to >= offset) f(this.lineAt(0, QueryType.ByPos, oracle, top$1, offset));
	}
...
var BigScaler = class BigScaler {
	...
	toDOM(n) {
		for (let i = 0, base$1 = 0, domBase = 0;; i++) {
...
var EditorView = class EditorView {
	...
	constructor(config = {}) {
		...
		this.updateState = 0;
		this.requestMeasure();
		if ((_a$1 = document.fonts) === null || _a$1 === void 0 ? void 0 : _a$1.ready) document.fonts.ready.then(() => {
	...
	measure(flush = true) {
		...
		try {
			for (let i = 0;; i++) {
				...
				for (let i$1 = 0; i$1 < measuring.length; i$1++) if (measured[i$1] != BadMeasure) try {
					let m = measuring[i$1];
					if (m.write) m.write(measured[i$1], this);
				} catch (e) {
					logException(this.state, e);
				}
...
function rectanglesForRange(view, className, range) {
	...
	function piece(left, top$1, right, bottom) {
		return new RectangleMarker(className, left - base$1.left, top$1 - base$1.top, Math.max(0, right - left), bottom - top$1);
	}
	function pieces({ top: top$1, bottom, horizontal }) {
		let pieces$1 = [];
		for (let i = 0; i < horizontal.length; i += 2) pieces$1.push(piece(horizontal[i], top$1, horizontal[i + 1], bottom));
		return pieces$1;
	}
...
var TooltipViewManager = class {
	...
	update(update, above) {
		...
		for (let i = 0; i < tooltips.length; i++) {
			...
			if (!tip) continue;
			for (let i$1 = 0; i$1 < this.tooltips.length; i$1++) {
				let other = this.tooltips[i$1];
				if (other && other.create == tip.create) known = i$1;
			}
			if (known < 0) {
...
var tooltipPlugin = /* @__PURE__ */ ViewPlugin.fromClass(class {
	...
	readMeasure() {
		...
		return {
			...
			pos: this.manager.tooltips.map((t$1, i) => {
...
var NodeSet = class NodeSet {
	constructor(types$1) {
		this.types = types$1;
		for (let i = 0; i < types$1.length; i++) if (types$1[i].id != i) throw new RangeError("Node type ids should correspond to array positions when creating a node set");
	}
	extend(...props) {
		let newTypes = [];
		...
		for (let parent = this;;) {
			for (let { children, positions } = parent._tree, e = dir > 0 ? children.length : -1; i != e; i += dir) {
				...
				if (next instanceof TreeBuffer) {
					...
					let mounted$1;
					if (!(mode & IterMode.IgnoreMounts) && (mounted$1 = MountedTree.get(next)) && !mounted$1.overlay) return new TreeNode(mounted$1.tree, start, i, parent);
...
function buildTree(data) {
	...
	function takeFlatNode(parentStart, minPos, children$1, positions$1) {
		...
		if (nodeCount) {
			...
			for (let i = nodes.length - 3, j = 0; i >= 0; i -= 3) {
				buffer$1[j++] = nodes[i];
				buffer$1[j++] = nodes[i + 1] - start;
				buffer$1[j++] = nodes[i + 2] - start;
				buffer$1[j++] = j;
			}
	...
	function makeRepeatLeaf(children$1, positions$1, base$1, i, from, to, type, lookAhead$1, contextHash$1) {
		let localChildren = [], localPositions = [];
...
function materialize(cursor) {
	...
	let base$1 = cursor.tree, i = base$1.children.indexOf(buffer);
	let buf = base$1.children[i], b = buf.buffer, newStack = [i];
	...
	base$1.children[i] = split(0, b.length, NodeType.none, 0, buf.length, stack.length - 1);
...
function syntaxIndentation(cx, ast, pos) {
	...
	if (inner != stack.node) {
		...
		for (let i = add$1.length - 1; i >= 0; i--) stack = {
			node: add$1[i],
			next: stack
		};
	}
...
function bracketDeco(state) {
	...
	for (let range of state.selection.ranges) {
		...
		if (match) decorations$1 = decorations$1.concat(config.renderMatch(match, state));
	}
...
function countCol(string$1, end, tabSize, startIndex = 0, startValue = 0) {
	...
	let n = startValue;
	for (let i = startIndex; i < end; i++) if (string$1.charCodeAt(i) == 9) n += tabSize - n % tabSize;
	else n++;
	return n;
}
...
export { getTooltip as $, syntaxHighlighting as A, codePointSize as At, NodeWeakMap as B, indentNodeProp as C, RangeSetBuilder as Ct, languageDataProp as D, Text as Dt, indentUnit as E, StateField as Et, DefaultBufferLength as F, Direction as G, Tree as H, IterMode as I, WidgetType as J, EditorView as K, NodeProp as L, Tag as M, countColumn as Mt, styleTags as N, findClusterBreak as Nt, matchBrackets as O, Transaction as Ot, tags as P, fromCodePoint as Pt, getPanel as Q, NodeSet as R, getIndentation as S, RangeSet as St, indentString as T, StateEffect as Tt, parseMixed as U, Parser as V, Decoration as W, drawSelection as X, crosshairCursor as Y, dropCursor as Z, foldInside as _, EditorSelection as _t, LanguageDescription as a, lineNumbers as at, foldService as b, MapMode as bt, StreamLanguage as c, rectangularSelection as ct, continuedIndent as d, showTooltip as dt, highlightActiveLine as et, defaultHighlightStyle as f, crelt as ft, foldGutter as g, CharCategory as gt, flatIndent as h, ChangeSet as ht, Language as i, keymap as it, syntaxTree as j, combineConfig as jt, sublanguageProp as k, codePointAt as kt, bracketMatching as l, runScopeHandlers as lt, delimitedIndent as m, ChangeDesc as mt, IndentContext as n, highlightSpecialChars as nt, LanguageSupport as o, logException as ot, defineLanguageFacet as p, Annotation as pt, ViewPlugin as q, LRLanguage as r, hoverTooltip as rt, ParseContext as s, placeholder as st, HighlightStyle as t, highlightActiveLineGutter as tt, bracketMatchingHandle as u, showPanel as ut, foldKeymap as v, EditorState as vt, indentOnInput as w, RangeValue as wt, getIndentUnit as x, Prec as xt, foldNodeProp as y, Facet as yt, NodeType as z };
