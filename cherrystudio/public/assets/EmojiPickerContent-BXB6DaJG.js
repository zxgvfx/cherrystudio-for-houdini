...
import { t as Flag } from "./flag-t57nERNM.js";
import { t as Hash } from "./hash-BQnIc4aZ.js";
import { t as Lightbulb } from "./lightbulb-B3H6XJ_t.js";
import { t as PawPrint } from "./paw-print-CoIKv-Xz.js";
import { t as Plane } from "./plane-_nbq9Uad.js";
import { t as Smile } from "./smile-BudUiYx_.js";
import { t as Trophy } from "./trophy-RohfwB6k.js";
import { t as Utensils } from "./utensils-Cq7PDpR5.js";
...
function nativeEmojiToUnified$1(emoji) {
...
function useGetEmojisByCategory() {
	...
	var suggested = import_react.useMemo(function() {
		var _getSuggested;
		return (suggestedEmojisConfig ? suggestedEmojisConfig.map(function(emoji) {
			return { unified: nativeEmojiToUnified$1(emoji) };
		}) : (_getSuggested = getSuggested(suggestedEmojisModeConfig)) != null ? _getSuggested : []).map(function(s) {
			return emojiByUnified(s.unified);
		}).filter(Boolean);
	}, [
		suggestedUpdated,
		suggestedEmojisConfig,
...
function useSuggestedEmojisConfig() {
	return usePickerConfig().suggestedEmojis;
}
...
var data_default = "" + new URL("data-CreAftJo.json", import.meta.url).href;
var data_default$1 = "" + new URL("data-D-bgPXER.json", import.meta.url).href;
var data_default$2 = "" + new URL("data-DKp8PYJD.json", import.meta.url).href;
var data_default$3 = "" + new URL("data-B-9igBij.json", import.meta.url).href;
var data_default$4 = "" + new URL("data-DcV17Et8.json", import.meta.url).href;
var data_default$5 = "" + new URL("data-Cqj2_8DB.json", import.meta.url).href;
var data_default$6 = "" + new URL("data-BLD79ES_.json", import.meta.url).href;
var data_default$7 = "" + new URL("data-64fdJDqZ.json", import.meta.url).href;
var data_default$8 = "" + new URL("data-CWxxLvBw.json", import.meta.url).href;
var DATA_URL_MAP = {
	"en-US": data_default$1,
	"zh-CN": data_default$7,
	"zh-TW": data_default$8,
	"de-DE": data_default,
	"el-GR": data_default$1,
	"es-ES": data_default$2,
	"fr-FR": data_default$3,
	"ja-JP": data_default$4,
	"pt-PT": data_default$5,
	"ro-RO": data_default$1,
	"ru-RU": data_default$6,
	"vi-VN": data_default$1
};
...
var nativeEmojiToUnified = (emoji) => Array.from(emoji).map((codePoint) => codePoint.codePointAt(0)?.toString(16).padStart(4, "0")).filter(Boolean).join("-");
var getEmojiNames = (record) => {
	const names = [
...
const convertEmojiRecords = (records) => {
	const emojis = {
		[Categories.CUSTOM]: [],
		[Categories.SMILEYS_PEOPLE]: [],
		[Categories.ANIMALS_NATURE]: [],
...
var loadEmojiDataUrl = (url) => {
	const cached = dataCache.get(url);
	if (cached) return cached;
	const promise = fetch(url).then((response) => {
		if (!response.ok) throw new Error(`Failed to load emoji data from ${url}: ${response.status} ${response.statusText}`);
		return response.json();
	}).then(convertEmojiRecords).catch((error) => {
...
const loadEmojiData = async (locale) => {
	const url = DATA_URL_MAP[locale];
	try {
		return await loadEmojiDataUrl(url);
	} catch (error) {
		if (url === "" + new URL("data-D-bgPXER.json", import.meta.url).href) throw error;
		return loadEmojiDataUrl(data_default$1);
	}
};
var EMOJI_VERSION_TESTS = [
	{
		emoji: "🫪",
		version: 17
	},
...
var BASELINE_EMOJI = "😀";
var ZWJ_CODE_POINT = "200d";
var ZWJ_UNSUPPORTED_WIDTH_RATIO = 1.8;
var EMOJI_FONT_FAMILY = "\"Twemoji Mozilla\",\"Apple Color Emoji\",\"Segoe UI Emoji\",\"Segoe UI Symbol\",\"Noto Color Emoji\",\"EmojiOne Color\",\"Android Emoji\",sans-serif";
var emojiSupportLevelPromise;
var unsupportedZwjEmojiIdsPromises = /* @__PURE__ */ new Map();
...
var hasMatchingColorEmojiFeatures = (left, right) => {
	const leftFeature = [...left].join(",");
	return leftFeature === [...right].join(",") && !leftFeature.startsWith("0,0,0,");
};
var isColorEmojiSupported = (emoji) => {
...
var determineEmojiSupportLevel = () => {
	try {
		for (const { emoji, version } of EMOJI_VERSION_TESTS) if (isColorEmojiSupported(emoji)) return version;
	} catch {}
	return EMOJI_VERSION_TESTS[0].version;
};
var runWhenIdle = (callback) => {
...
const detectEmojiSupportLevel = () => {
	if (!emojiSupportLevelPromise) emojiSupportLevelPromise = new Promise((resolve) => {
		runWhenIdle(() => resolve(determineEmojiSupportLevel()));
	});
	return emojiSupportLevelPromise;
};
var unifiedToNativeEmoji = (unified) => unified.split("-").map((codePoint) => String.fromCodePoint(Number.parseInt(codePoint, 16))).join("");
var getZwjEmojiIds = (emojiData) => {
	const ids = Object.values(emojiData.emojis).flat().map((emoji) => emoji.u).filter((unified) => unified.includes(ZWJ_CODE_POINT));
	return [...new Set(ids)];
};
var createMeasurementNode = () => {
	if (!document.body) return void 0;
...
var determineUnsupportedZwjEmojiIds = (emojiIds) => {
	try {
...
const detectUnsupportedZwjEmojiIds = (emojiData) => {
...
var MAX_RECENT_EMOJIS = 32;
const useRecentEmojis = () => {
	const [recent, setRecent] = usePersistCache("ui.emoji.recently_used");
	return {
		recent,
		pushRecent: (0, import_react.useCallback)((emoji) => {
			setRecent((prev) => [emoji, ...prev.filter((item) => item !== emoji)].slice(0, MAX_RECENT_EMOJIS));
		}, [setRecent]),
		clearRecent: (0, import_react.useCallback)(() => {
			setRecent([]);
		}, [setRecent])
	};
};
var import_jsx_runtime = /* @__PURE__ */ __toESM(require_jsx_runtime());
var logger = loggerService.withContext("EmojiPicker");
var CHERRY_PICKER_STYLE = {
...
var CATEGORY_ICON_CLASS = "size-4.5";
var CATEGORY_ICONS = {
	[Categories.SUGGESTED]: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Clock3, { className: CATEGORY_ICON_CLASS }),
	[Categories.SMILEYS_PEOPLE]: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Smile, { className: CATEGORY_ICON_CLASS }),
	[Categories.ANIMALS_NATURE]: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(PawPrint, { className: CATEGORY_ICON_CLASS }),
	[Categories.FOOD_DRINK]: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Utensils, { className: CATEGORY_ICON_CLASS }),
	[Categories.TRAVEL_PLACES]: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Plane, { className: CATEGORY_ICON_CLASS }),
	[Categories.ACTIVITIES]: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Trophy, { className: CATEGORY_ICON_CLASS }),
	[Categories.OBJECTS]: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Lightbulb, { className: CATEGORY_ICON_CLASS }),
	[Categories.SYMBOLS]: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Hash, { className: CATEGORY_ICON_CLASS }),
	[Categories.FLAGS]: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Flag, { className: CATEGORY_ICON_CLASS })
};
var EmojiPickerContent = ({ onEmojiClick }) => {
	const { t, i18n } = useTranslation();
	const locale = i18n.language;
	const [emojiData, setEmojiData] = (0, import_react.useState)();
	const [emojiVersion, setEmojiVersion] = (0, import_react.useState)();
	const [hiddenEmojis, setHiddenEmojis] = (0, import_react.useState)([]);
	const { recent, pushRecent } = useRecentEmojis();
	(0, import_react.useEffect)(() => {
		let cancelled = false;
		detectEmojiSupportLevel().then((version) => {
	...
	(0, import_react.useEffect)(() => {
		...
		loadEmojiData(locale).then((data) => {
			...
			if (!cancelled) setEmojiData(void 0);
		});
		return () => {
			cancelled = true;
		};
	...
	(0, import_react.useEffect)(() => {
		...
		detectUnsupportedZwjEmojiIds(emojiData).then((emojiIds) => {
	...
	const categories = (0, import_react.useMemo)(() => CATEGORY_ORDER.map((category) => ({
		category,
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		"data-ui": "ui.emoji-picker-content",
		className: "h-88 max-h-[min(22rem,calc(100vh-6rem))] w-80 max-w-[calc(100vw-2rem)] rounded-lg bg-popover text-popover-foreground",
...
var EmojiPickerContent_default = EmojiPickerContent;
export { EmojiPickerContent_default as default };
