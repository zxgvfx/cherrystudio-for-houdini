...
const apiGateway = {
	...
	"required": {
		"confirm": "启用",
		"description": "该智能体使用的模型需要通过 Cherry Studio 的本地 API Gateway 桥接。启用后网关会在以后启动时自动运行,你可以随时在设置中再次关闭。",
		"title": "启用 API Gateway?"
	},
	"status": {
		"running": "运行中",
		"stopped": "已停止"
...
const assistants = {
	...
	"settings": {
		...
		"regular_phrases": {
			"add": "添加短语",
			"contentLabel": "内容",
			"contentPlaceholder": "请输入短语内容。支持使用 ${变量}；按 Tab 可在变量间快速定位。例如：\n帮我规划从 ${出发地} 到 ${目的地} 的路线，然后发送到 ${邮箱}。",
			"delete": "删除短语",
...
const error = {
	"api_gateway_required": "该模型需要通过 Cherry Studio 的本地 API Gateway 桥接,但网关当前已禁用。启用后才能运行该智能体。",
	...
	"chat": {
		...
		"quota_exceeded": "您今日的 {{quota}} 免费配额已用尽，请前往 <provider>{{provider}}</provider> 获取并配置 API 密钥后继续使用",
	...
	"diagnosis": {
		...
		"free_model_unavailable": "AI 诊断暂时不可用",
		"go_to_settings": "前往设置",
		"knowledge": "知识库向量化失败",
		...
		"permission": "服务商拒绝了此请求。请查看错误详情，并确认账户套餐、API 密钥权限或该资源的访问权限",
	...
	"toolInput": "工具输入",
	"toolName": "工具名",
	"tool_call_limit_reached": "助手在生成最终回复前已达到工具调用上限。请在助手设置中调高“最大工具调用轮次”，或缩小任务范围。",
...
const file_preview = {
	...
	"explode": "炸开",
	"explode_hint": "分开网格部件，检查分割是否正确",
	"explode_reset": "重置炸开程度",
	"explode_unavailable": "这个模型没有可分开的部件",
	"html": {
		"empty": {
...
const knowledge = {
	...
	"data_source": {
		...
		"add_dialog": {
			...
			"note": {
				"create": {
					"content_label": "正文",
					"content_placeholder": "在此输入笔记内容…",
					"title_label": "标题",
					"title_placeholder": "为这篇笔记取个名字"
				},
	...
	"rag": {
		...
		"processor_not_downloaded": "未下载",
		"processor_unreachable": "服务未启动",
		"rerank_disabled": "不使用",
		"rerank_model": "重排模型",
		"reset_action": "恢复默认",
		"reset_defaults": "恢复默认",
...
const models = {
	...
	"price": {
		"add_tier": "添加价格档位",
		"cache_fallback_help": "缓存价格留空时回退到本档输入价格；输入 0 表示免费。",
		"cache_read": "缓存读取价格",
		"cache_write": "缓存写入价格",
		"cost": "花费",
		"currency": "币种",
		"custom": "自定义",
		"field_for_tier": "第 {{index}} 档{{field}}",
		"input": "输入价格",
		"million_tokens": "百万 Token",
		"min_input_tokens": "生效输入 Token 数",
		"min_input_tokens_help": "边界包含该值，且必须大于上一档。",
		"output": "输出价格",
		"price": "价格",
		"remove_tier": "删除第 {{index}} 个价格档位",
		"tier": "档位 {{index}}",
		"tier_from": "从 {{boundary}} 个输入 Token 起（含）",
		"use_input_price": "使用输入价格",
		"validation_min_input_tokens": "请输入正整数。",
		"validation_min_input_tokens_order": "本档起点必须大于上一档。",
		"validation_price": "请输入大于或等于 0 的有效价格。"
	},
	"reasoning": "推理",
	"rerank_model": "重排模型",
...
const notes = {
	...
	"settings": {
		...
		"display": {
			...
			"compress_content_description": "开启后将限制每行字数，减少屏幕上显示的内容，但能提高长段落的可读性。",
			"default_font": "默认字体",
			"font_size": "字体大小",
			...
			"font_title": "字体设置",
			"line_breaks": "换行方式",
			"line_breaks_description": "开启后单个换行符将渲染为新的一行（类似 Obsidian）。关闭时换行符会被折叠为空格，直到出现空行才会分段。",
			"serif_font": "衬线字体",
...
const openclaw = {
	...
	"node_missing": {
		"description": "OpenClaw 需要 Node.js 22 或更高版本。请先安装 Node.js，然后再次点击安装。",
...
const ovms = {
	...
	"description": "<div><p>1. 下载 OV 模型。</p><p>2. 在“管理”中添加模型。</p><p>仅支持 Windows。</p><p>OVMS 安装路径：'%USERPROFILE%\\.cherrystudio\\ovms'。</p><p>请参阅 <a href=\"https://github.com/openvinotoolkit/model_server/blob/c55551763d02825829337b62c2dcef9339706f79/docs/deploying_server_baremetal.md\">Intel OVMS 指南</a>。</p></div>",
	"download": {
		"button": "下载",
		"error": "下载失败",
		"model_id": {
...
const selection = {
	"action": {
		...
		"prompt": {
			...
			"refine": "请优化或润色 INPUT XML 元素中的用户输入，同时保持原文的含义和完整性。要求：输出语言应与用户输入相同；不要解释本提示词，直接给出结果；不要输出 XML 标签，直接输出优化后的内容：\n\n<INPUT>{{text}}</INPUT>",
...
const warning = { "missing_provider": "服务商不存在，已回退到默认服务商 {{provider}}。这可能导致问题。" };
const words = {
	"knowledgeGraph": "知识图谱",
	"quit": "退出",
	"show_window": "显示窗口",
...
var zh_cn_default = {
	...
	"import": {
		...
		"claude": {
			...
			"button": "选择文件",
			"description": "导入文字、思考过程和工具调用，不包含图片和附件",
			"error": {
				"invalid_json": "无效的 JSON 文件格式",
				"no_conversations": "文件中未找到任何对话",
				"no_valid_conversations": "没有可导入的有效对话",
