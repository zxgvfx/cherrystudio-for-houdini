...
var DocTile = class extends CompositeTile {
	...
	blockTiles(f) {
		for (let stack = [], cur = this, i$1 = 0, pos = 0;;) if (i$1 == cur.children.length) {
			if (!stack.length) return;
			cur = cur.parent;
			if (cur.breakAfter) pos++;
			i$1 = stack.pop();
		} else {
			let next = cur.children[i$1++];
...
var TileBuilder = class {
	...
	updateBlockWrappers() {
		...
		for (let cur = this.blockWrappers; cur.value && cur.from <= this.pos; cur.next()) if (cur.to >= this.pos) {
			let wrap = new OpenWrapper(cur.from, cur.to, cur.value, cur.rank), i$1 = this.wrappers.length;
...
var TileCache = class {
	...
	clear() {
		for (let i$1 = 0; i$1 < this.buckets.length; i$1++) this.buckets[i$1].length = this.index[i$1] = 0;
	}
};
...
var InlineCoordsScan = class {
	...
	scan(positions, getRects) {
		...
		search: while (lo < hi) {
			...
			if (rects) for (let i$1 = 0; i$1 < rects.length; i$1++) {
				let rect = rects[i$1], side = 0;
	...
	scanText(tile, offset) {
		let positions = [];
		for (let i$1 = 0; i$1 < tile.length; i$1 = findClusterBreak(tile.text, i$1)) positions.push(offset + i$1);
		positions.push(offset + tile.length);
		let scan = this.scan(positions, (i$1) => {
			let off = positions[i$1] - offset, end = positions[i$1 + 1] - offset;
	...
	scanTile(tile, offset) {
		...
		for (let i$1 = 0, pos$1 = offset; i$1 < tile.children.length; i$1++) positions.push(pos$1 += tile.children[i$1].length);
		let scan = this.scan(positions, (i$1) => {
			let child$1 = tile.children[i$1];
	...
	nextChild(i$1, dir, pos, side, mode = 0) {
		for (let parent = this;;) {
			for (let { children, positions } = parent._tree, e = dir > 0 ? children.length : -1; i$1 != e; i$1 += dir) {
				let next = children[i$1], start = positions[i$1] + parent.from, mounted;
				...
				if (next instanceof TreeBuffer) {
					...
					let mounted$1;
					if (!(mode & IterMode.IgnoreMounts) && (mounted$1 = MountedTree.get(next)) && !mounted$1.overlay) return new TreeNode(mounted$1.tree, start, i$1, parent);
...
function cutTree(lang, tree, from, to, inside) {
	...
	for (let i$1 = tree.children.length - 1; i$1 >= 0; i$1--) {
		...
		if (pos < to && child instanceof Tree) {
			...
			return !inside ? inner : new Tree(tree.type, tree.children.slice(0, i$1).concat(inner), tree.positions.slice(0, i$1 + 1), pos + inner.length);
		}
	}
	return null;
}
...
export { getTooltip as $, syntaxHighlighting as A, Transaction as At, NodeWeakMap as B, indentNodeProp as C, Prec as Ct, languageDataProp as D, StateEffect as Dt, indentUnit as E, RangeValue as Et, DefaultBufferLength as F, findClusterBreak as Ft, Direction as G, Tree as H, IterMode as I, fromCodePoint as It, WidgetType as J, EditorView as K, NodeProp as L, Tag as M, codePointSize as Mt, styleTags as N, combineConfig as Nt, matchBrackets as O, StateField as Ot, tags as P, countColumn as Pt, getPanel as Q, NodeSet as R, getIndentation as S, MapMode as St, indentString as T, RangeSetBuilder as Tt, parseMixed as U, Parser as V, Decoration as W, drawSelection as X, crosshairCursor as Y, dropCursor as Z, foldInside as _, ChangeSet as _t, LanguageDescription as a, lineNumbers as at, foldService as b, EditorState as bt, StreamLanguage as c, rectangularSelection as ct, continuedIndent as d, showTooltip as dt, highlightActiveLine as et, defaultHighlightStyle as f, crelt as ft, foldGutter as g, ChangeDesc as gt, flatIndent as h, Annotation as ht, Language as i, keymap as it, syntaxTree as j, codePointAt as jt, sublanguageProp as k, Text as kt, bracketMatching as l, runScopeHandlers as lt, delimitedIndent as m, keyName as mt, IndentContext as n, highlightSpecialChars as nt, LanguageSupport as o, logException as ot, defineLanguageFacet as p, base as pt, ViewPlugin as q, LRLanguage as r, hoverTooltip as rt, ParseContext as s, placeholder as st, HighlightStyle as t, highlightActiveLineGutter as tt, bracketMatchingHandle as u, showPanel as ut, foldKeymap as v, CharCategory as vt, indentOnInput as w, RangeSet as wt, getIndentUnit as x, Facet as xt, foldNodeProp as y, EditorSelection as yt, NodeType as z };
