...
function Match(text$1, schema, index, lastIndex) {
...
LinkifyIt.prototype.test = function test(text$1) {
	...
	if (this.__opts__.fuzzyLink && this.__compiled__["http:"]) {
		if (text$1.search(this.re.host_fuzzy_test) >= 0) {
			if (text$1.match(this.__opts__.fuzzyIP ? this.re.link_fuzzy : this.re.link_no_ip_fuzzy) !== null) return true;
		}
	}
	if (this.__opts__.fuzzyEmail && this.__compiled__["mailto:"]) {
		if (text$1.indexOf("@") >= 0) {
			if (text$1.match(this.re.email_fuzzy) !== null) return true;
		}
	}
	return false;
};
...
LinkifyIt.prototype.match = function match(text$1) {
	...
	if (this.__opts__.fuzzyLink && this.__compiled__["http:"]) {
		...
		re.lastIndex = 0;
		while ((m = re.exec(text$1)) !== null) type_fuzzy_link.push({
			schema: "",
			index: m.index + m[1].length,
	...
	if (this.__opts__.fuzzyEmail && this.__compiled__["mailto:"]) {
		...
		re.lastIndex = 0;
		while ((m = re.exec(text$1)) !== null) type_fuzzy_email.push({
			schema: "mailto:",
			index: m.index + m[1].length,
			lastIndex: m.index + m[0].length
		});
	...
	for (;;) {
		...
		const match$1 = new Match(text$1, candidate.schema, candidate.index, candidate.lastIndex);
		this.__compiled__[match$1.schema].normalize(match$1, this);
		result.push(match$1);
