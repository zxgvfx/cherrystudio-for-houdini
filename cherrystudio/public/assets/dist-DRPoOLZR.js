...
import { n as keyName, t as base } from "./w3c-keyname-d7Yk5myI.js";
...
var Schema = class {
	constructor(spec) {
		...
		instanceSpec.nodes = dist_default.from(spec.nodes), instanceSpec.marks = dist_default.from(spec.marks || {}), this.nodes = NodeType$1.compile(this.spec.nodes, this);
...
function deleteBarrier(state, $cut, dispatch, dir) {
	...
	if (canDelAfter && (conn = (match = before.contentMatchAt(before.childCount)).findWrapping(after.type)) && match.matchType(conn[0] || after.type).validEnd) {
		if (dispatch) {
			...
			for (let i = conn.length - 1; i >= 0; i--) wrap$1 = Fragment.from(conn[i].create(null, wrap$1));
	...
	if (canDelAfter && textblockAt(after, "start", true) && textblockAt(before, "end")) {
		...
		if (at.canReplace(at.childCount, at.childCount, afterText.content)) {
			if (dispatch) {
				...
				for (let i = wrap$1.length - 1; i >= 0; i--) end = Fragment.from(wrap$1[i].copy(end));
...
function withWrappers(node, wrap$1, from = 0) {
	for (let i = wrap$1.length - 1; i >= from; i--) node = wrap$1[i].create(null, Fragment.from(node));
	return node;
}
...
function objectIncludes(object1, object2, options = { strict: true }) {
	const keys$1 = Object.keys(object2);
	if (!keys$1.length) return true;
...
var keyboardShortcut = (name) => ({ editor, view, tr, dispatch }) => {
	const keys$1 = normalizeKeyName(name).split(/-(?!$)/);
	const key = keys$1.find((item) => ![
		"Alt",
		"Ctrl",
		"Meta",
		"Shift"
	...
	const event = new KeyboardEvent("keydown", {
		key: key === "Space" ? " " : key,
		altKey: keys$1.includes("Alt"),
		ctrlKey: keys$1.includes("Ctrl"),
		metaKey: keys$1.includes("Meta"),
		shiftKey: keys$1.includes("Shift"),
		bubbles: true,
		cancelable: true
	});
...
export { Decoration as $, getRenderedAttributes as A, markInputRule as B, generateJSON as C, getMarksBetween as D, getExtensionField as E, isFirefox as F, parseIndentedBlocks as G, marksEqual as H, isNodeActive as I, sortExtensions as J, posToDOMRect as K, isNodeEmpty as L, getUpdatedPosition as M, isAtEndOfNode as N, getNodeAtPosition as O, isAtStartOfNode as P, keydownHandler as Q, isNodeSelection as R, flattenExtensions as S, getChangedRanges as T, mergeAttributes as U, markPasteRule as V, nodeInputRule as W, textblockTypeInputRule as X, textInputRule as Y, wrappingInputRule as Z, decodeHtmlEntities as _, MappablePosition as a, Selection as at, findChildrenInRange as b, NodeView as c, Mapping as ct, callOrReturn as d, Fragment as dt, DecorationSet as et, canInsertNode as f, Node as ft, createStyleTag as g, createInlineMarkdownSpec as h, NarrowLayout_default as ht, InputRule as i, PluginKey as it, getSchema as j, getNodeType as k, ResizableNodeView as l, Transform as lt, commands_exports as m, Slice as mt, Editor as n, NodeSelection as nt, Mark as o, SelectionRange as ot, combineTransactionSteps as p, NodeRange as pt, renderNestedMarkdownContent as q, Extension as r, Plugin as rt, Node3 as s, TextSelection as st, Suggestion as t, AllSelection as tt, attrsEqual as u, dropPoint as ut, encodeHtmlEntities as v, getAttributes as w, findParentNodeClosestToPos as x, findChildren as y, isNodeViewSelected as z };
