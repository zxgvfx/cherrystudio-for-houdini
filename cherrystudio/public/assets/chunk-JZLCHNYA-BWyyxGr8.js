...
import { A as parseGenericTypes, I as sanitizeText, L as sanitizeText3, N as renderKatexSanitized, O as hasKatex, b as getConfig2, d as defaultConfig_default, h as evaluate, s as common_default, y as getConfig } from "./chunk-ABZYJK2D-BJEZmKZ0.js";
import { a as decodeEntities, p as parseFontSize, r as calculateTextWidth, u as handleUndefinedAttr } from "./chunk-S3R3BYOJ-SD49tN4s.js";
import { n as createText, r as getIconSVG } from "./chunk-JA3XYJ7Z-DT_6yhAz.js";
import { t as getSubGraphTitleMargins } from "./chunk-CVBHYZKI-BM3mjY3h.js";
import { a as userNodeOverrides, i as styles2String, r as solidStateFill, t as compileStyles } from "./chunk-ATLVNIR6-CA8VX9_O.js";
...
async function addHtmlLabel(node) {
	...
	const labelSpan = "<span class=\"" + (node.isNode ? "nodeLabel" : "edgeLabel") + "\" " + (node.labelStyle ? "style=\"" + node.labelStyle + "\"" : "") + ">" + label + "</span>";
...
var shapes = {
	...
	roundedWithTitle: /* @__PURE__ */ __name(async (parent, node) => {
		...
		if (node.look === "handDrawn") {
			...
			rect2.attr("class", "outer").attr("x", x$1).attr("y", y$1).attr("width", width).attr("height", height).attr("data-look", node.look);
	...
	divider: /* @__PURE__ */ __name((parent, node) => {
		...
		let rect2;
		if (node.look === "handDrawn") {
			const roughOuterNode = at.svg(shapeSvg).rectangle(x$1, y$1, width, height, {
				fill: "lightgrey",
				roughness: .5,
				strokeLineDash: [5],
				stroke: nodeBorder,
			...
			rect2 = outerRectG.insert("rect", ":first-child");
			rect2.attr("class", "divider").attr("x", x$1).attr("y", y$1).attr("width", width).attr("height", height).attr("data-look", node.look);
		}
		node.height = rect2.node().getBBox().height;
		node.offsetX = 0;
		node.offsetY = 0;
...
function intersectLine(p1, p2, q1, q2) {
	{
		...
		let num = b1 * c2 - b2 * c1;
		const x$1 = num < 0 ? (num - offset) / denom : (num + offset) / denom;
		num = a2 * c1 - a1 * c2;
		return {
			x: x$1,
...
function generateArcPoints(x1, y1, x2, y2, rx, ry, clockwise) {
	...
	const points = [];
	for (let i$1 = 0; i$1 < numPoints; i$1++) {
		const angle2 = startAngle + i$1 / (numPoints - 1) * angleRange;
...
async function note(parent, node, { config: { themeVariables } }) {
	const { labelStyles, nodeStyles } = styles2String(node);
	node.labelStyle = labelStyles;
	if (!(node.useHtmlLabels || getConfig().flowchart?.htmlLabels !== false)) node.centerLabel = true;
	const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node));
...
async function question(parent, node) {
	...
	node.labelStyle = labelStyles;
	const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node));
	const s$1 = bbox.width + node.padding + (bbox.height + node.padding);
	const adjustment = .5;
	const points = [
		{
			x: s$1 / 2,
...
async function roundedRect(parent, node) {
	...
	const rc = at.svg(shapeSvg);
	const options = userNodeOverrides(node, {});
	if (node.stroke) options.stroke = node.stroke;
	if (node.look !== "handDrawn") {
		options.roughness = 0;
		options.fillStyle = "solid";
	}
...
async function erBox(parent, node) {
	...
	for (const [i$1, row] of rows.entries()) {
		const isEven = (i$1 + 1) % 2 === 0 && row.yOffset !== 0;
	...
	if (nodeStyles && node.look !== "handDrawn") {
		const strokeStyles = nodeStyles.split(";")?.filter((e$1) => {
...
async function bang(parent, node) {
	...
	const w$1 = bbox.width + 10 * halfPadding;
	const h$1 = bbox.height + 8 * halfPadding;
	const r$1 = .15 * w$1;
	const { cssStyles } = node;
	const minWidth = bbox.width + 20;
	const minHeight = bbox.height + 20;
	const effectiveWidth = Math.max(w$1, minWidth);
	const effectiveHeight = Math.max(h$1, minHeight);
	label.attr("transform", `translate(${-bbox.width / 2}, ${-bbox.height / 2})`);
	let bangElem;
	const path = `M0 0 
    a${r$1},${r$1} 1 0,0 ${effectiveWidth * .25},${-1 * effectiveHeight * .1}
    a${r$1},${r$1} 1 0,0 ${effectiveWidth * .25},0
    a${r$1},${r$1} 1 0,0 ${effectiveWidth * .25},0
    a${r$1},${r$1} 1 0,0 ${effectiveWidth * .25},${effectiveHeight * .1}

    a${r$1},${r$1} 1 0,0 ${effectiveWidth * .15},${effectiveHeight * .33}
    a${r$1 * .8},${r$1 * .8} 1 0,0 0,${effectiveHeight * .34}
    a${r$1},${r$1} 1 0,0 ${-1 * effectiveWidth * .15},${effectiveHeight * .33}

    a${r$1},${r$1} 1 0,0 ${-1 * effectiveWidth * .25},${effectiveHeight * .15}
    a${r$1},${r$1} 1 0,0 ${-1 * effectiveWidth * .25},0
    a${r$1},${r$1} 1 0,0 ${-1 * effectiveWidth * .25},0
    a${r$1},${r$1} 1 0,0 ${-1 * effectiveWidth * .25},${-1 * effectiveHeight * .15}

    a${r$1},${r$1} 1 0,0 ${-1 * effectiveWidth * .1},${-1 * effectiveHeight * .33}
    a${r$1 * .8},${r$1 * .8} 1 0,0 0,${-1 * effectiveHeight * .34}
    a${r$1},${r$1} 1 0,0 ${effectiveWidth * .1},${-1 * effectiveHeight * .33}
  H0 V0 Z`;
	if (node.look === "handDrawn") {
		const rc = at.svg(shapeSvg);
		const options = userNodeOverrides(node, {});
...
async function cloud(parent, node) {
	...
	node.labelStyle = labelStyles;
	const { shapeSvg, bbox, halfPadding, label } = await labelHelper(parent, node, getNodeClasses(node));
	const w$1 = bbox.width + 2 * halfPadding;
	const h$1 = bbox.height + 2 * halfPadding;
	const r1 = .15 * w$1;
	const r2 = .25 * w$1;
	const r3 = .35 * w$1;
	const r4 = .2 * w$1;
	...
	let cloudElem;
	const path = `M0 0 
    a${r1},${r1} 0 0,1 ${w$1 * .25},${-1 * w$1 * .1}
    a${r3},${r3} 1 0,1 ${w$1 * .4},${-1 * w$1 * .1}
    a${r2},${r2} 1 0,1 ${w$1 * .35},${w$1 * .2}

    a${r1},${r1} 1 0,1 ${w$1 * .15},${h$1 * .35}
    a${r4},${r4} 1 0,1 ${-1 * w$1 * .15},${h$1 * .65}

    a${r2},${r1} 1 0,1 ${-1 * w$1 * .25},${w$1 * .15}
    a${r3},${r3} 1 0,1 ${-1 * w$1 * .5},0
    a${r1},${r1} 1 0,1 ${-1 * w$1 * .25},${-1 * w$1 * .15}

    a${r1},${r1} 1 0,1 ${-1 * w$1 * .1},${-1 * h$1 * .35}
    a${r4},${r4} 1 0,1 ${w$1 * .1},${-1 * h$1 * .65}
  H0 V0 Z`;
	if (node.look === "handDrawn") {
		const rc = at.svg(shapeSvg);
		const options = userNodeOverrides(node, {});
	...
	label.attr("transform", `translate(${-bbox.width / 2}, ${-bbox.height / 2})`);
	cloudElem.attr("transform", `translate(${-w$1 / 2}, ${-h$1 / 2})`);
...
async function defaultMindmapNode(parent, node) {
	...
	node.labelStyle = labelStyles;
	const { shapeSvg, bbox, halfPadding, label } = await labelHelper(parent, node, getNodeClasses(node));
	const w$1 = bbox.width + 8 * halfPadding;
	const h$1 = bbox.height + 2 * halfPadding;
	const rd = 5;
	const rectPath = `
    M${-w$1 / 2} ${h$1 / 2 - rd}
  ...
  `;
	...
	shapeSvg.append("line").attr("class", "node-line-").attr("x1", -w$1 / 2).attr("y1", h$1 / 2).attr("x2", w$1 / 2).attr("y2", h$1 / 2);
	label.attr("transform", `translate(${-bbox.width / 2}, ${-bbox.height / 2})`);
