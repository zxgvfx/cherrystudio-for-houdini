...
import { r as DefaultPreferences } from "./PreferenceService-ay5pWhVK.js";
...
var getEventKeyToken = (event) => {
	...
	return token && !isShortcutModifier(token) ? token : void 0;
};
...
var isPlatformBindingMap = (defaultBinding) => !Array.isArray(defaultBinding);
var getRuleBaseBinding = (defaultBinding) => isPlatformBindingMap(defaultBinding) ? defaultBinding.default : defaultBinding;
var getRulePlatformBinding = (defaultBinding, platform$1) => {
	if (!platform$1 || !isPlatformBindingMap(defaultBinding)) return;
	const binding = defaultBinding[platform$1];
	return binding ? normalizeShortcutBinding(binding) : void 0;
};
var getSharedDefaultBinding = (rule) => {
	const fallback = DefaultPreferences.default[rule.preferenceKey];
	return fallback?.binding?.length ? normalizeShortcutBinding(fallback.binding) : getRuleBaseBinding(rule.defaultBinding);
};
var getDefaultShortcutPreferenceForRule = (rule, platform$1) => {
	const fallback = DefaultPreferences.default[rule.preferenceKey];
	return {
		binding: getRulePlatformBinding(rule.defaultBinding, platform$1) ?? getSharedDefaultBinding(rule),
		enabled: typeof fallback?.enabled === "boolean" ? fallback.enabled : true
	};
};
var resolvePreferredBinding = (rule, preference, platform$1) => {
	if (preference == null) return;
	if (!preference.binding?.length) return [];
	const binding = normalizeShortcutBinding(preference.binding);
	const platformBinding = getRulePlatformBinding(rule.defaultBinding, platform$1);
	if (platformBinding && shortcutBindingMatches(binding, getSharedDefaultBinding(rule))) return platformBinding;
	return binding;
};
const getCommandDefaultShortcutPreference = (command, platform$1) => {
	const rule = findKeybindingRule(command);
	if (!rule) return;
	return getDefaultShortcutPreferenceForRule(rule, platform$1);
};
const resolveCommandShortcutPreference = (command, preference, platform$1) => {
	const rule = findKeybindingRule(command);
	if (!rule) return;
	const fallback = getDefaultShortcutPreferenceForRule(rule, platform$1);
	return {
		binding: resolvePreferredBinding(rule, preference, platform$1) ?? fallback.binding,
...
const resolveCommandKeybinding = ({ command, preference, context, platform: platform$1 }) => {
	...
	const shortcutPreference = resolveCommandShortcutPreference(command, preference, platform$1);
...
const findKeybindingConflicts = ({ command, preference, preferences, platform: platform$1, rules = REGISTERED_KEYBINDINGS }) => {
	...
	for (const rule of rules) {
		...
		const conflictingPreference = resolveCommandShortcutPreference(rule.command, preferenceLookup[rule.command], platform$1);
...
export { useCommandRuntime as C, rendererPlatform as D, ContextKeySnapshotContext as E, useCommandContextReader as O, useCommandMenuPresentationMode as S, ContextKeyRegisterContext as T, findKeybindingRule as _, useCommandShortcuts as a, CommandSharedPreferencesContext as b, resolveCommandByKeybinding as c, formatShortcutDisplay as d, getShortcutBindingFromKeyboardEvent as f, findCommandDefinition as g, REGISTERED_KEYBINDINGS as h, getAllShortcutDefaultPreferences as i, useCommandContextSnapshot as k, convertKeyToAccelerator as l, normalizeShortcutToken as m, useResolvedCommand as n, resolveMenuPresentationMode as o, isValidShortcut as p, getCommandShortcutLabel as r, findKeybindingConflicts as s, useResolvedCommandMenu as t, formatKeyDisplay as u, ContextKeyService as v, useCommandShortcutPreferences as w, useCommandHandler as x, CommandRuntimeContext as y };
