...
import { i as getForegroundColor } from "./style-qqUWb85F.js";
var colors = {
	aliceblue: [
		240,
		248,
...
function buildGraph() {
	const graph = {};
	const models$1 = Object.keys(conversions_default);
	for (let { length } = models$1, i = 0; i < length; i++) graph[models$1[i]] = {
		distance: -1,
		parent: null
	};
	return graph;
...
function route(fromModel) {
	...
	for (let { length } = models$1, i = 0; i < length; i++) {
...
function useUserTheme() {
	...
	const setOptionalCssVar = (name, value) => {
		if (value?.trim()) {
	...
	const initUserTheme = (theme = { colorPrimary }) => {
		const colorPrimary$1 = color_default(theme.colorPrimary);
		document.documentElement.style.setProperty("--cs-theme-primary", colorPrimary$1.toString());
		document.documentElement.style.setProperty("--cs-theme-primary-foreground", getForegroundColor(colorPrimary$1.hex()));
		setOptionalCssVar("--app-user-font-family", userFontFamily);
		setOptionalCssVar("--app-user-code-font-family", userCodeFontFamily);
	};
	return {
		colorPrimary: color_default(colorPrimary),
		initUserTheme,
		userTheme: {
			colorPrimary,
			userFontFamily,
...
function hasV1CustomCssMarker(cssText) {
	return cssText === "/* cherry-studio:custom-css:v1 */" || cssText?.startsWith(`/* cherry-studio:custom-css:v1 */\n`) === true || cssText?.startsWith(`/* cherry-studio:custom-css:v1 */\r\n`) === true;
}
export { useUserTheme as n, hasV1CustomCssMarker as t };
