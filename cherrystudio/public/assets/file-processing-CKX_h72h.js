...
var documentProcessingFieldClassName = "[&_input[data-slot=input]]:h-8 [&_input[data-slot=input]]:rounded-lg [&_input[data-slot=input]]:border-border-subtle [&_input[data-slot=input]]:bg-muted/30 [&_input[data-slot=input]]:px-2.5 [&_input[data-slot=input]]:shadow-none [&_input[data-slot=input]:focus-visible]:ring-[1px] [&_input[data-slot=input]:focus-visible]:ring-ring/35";
var DocumentProcessingSettings = () => {
	const { t } = useTranslation();
	const { theme: themeMode } = useTheme();
	const { defaultDocumentProcessor, processors, setApiKeys, setCapabilityField, setDefaultProcessor, setLanguageOptions } = useFileProcessingPreferences();
	const availableProcessors = useAvailableFileProcessors();
	const visibleProcessorIds = (0, import_react.useMemo)(() => availableProcessors.status === "ready" || !defaultDocumentProcessor ? availableProcessors.processorIds : new Set([defaultDocumentProcessor]), [
		availableProcessors.processorIds,
		availableProcessors.status,
		defaultDocumentProcessor
	]);
	const menuEntries = (0, import_react.useMemo)(() => getFeatureSections(processors, visibleProcessorIds).find((section) => section.feature === "document_to_markdown")?.entries ?? EMPTY_MENU_ENTRIES, [processors, visibleProcessorIds]);
	const [activeKey, setActiveKey] = (0, import_react.useState)(() => menuEntries.find((entry) => entry.processor.id === defaultDocumentProcessor)?.key ?? menuEntries[0]?.key ?? "");
	(0, import_react.useEffect)(() => {
		setActiveKey(menuEntries.find((entry) => entry.processor.id === defaultDocumentProcessor)?.key ?? menuEntries[0]?.key ?? "");
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SettingsContentColumn, {
		theme: themeMode,
		innerClassName: documentProcessingFieldClassName,
...
var SplitComponent = DocumentProcessingSettings;
