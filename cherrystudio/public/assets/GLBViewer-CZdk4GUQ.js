...
import { n as dt } from "./styled-components.browser.esm-uS3mDXI2.js";
import { t as require_jsx_runtime } from "./jsx-runtime-DuRsdr50.js";
import { $ as NearestMipmapLinearFilter, A as InterpolateLinear, B as LoaderUtils, C as ImageBitmapLoader, Ct as TriangleStripDrawMode, D as InterleavedBufferAttribute, Dt as Vector3, E as InterleavedBuffer, Et as Vector2, F as LinearFilter, G as Mesh, H as MathUtils, I as LinearMipmapLinearFilter, K as MeshBasicMaterial, L as LinearMipmapNearestFilter, M as LineBasicMaterial, N as LineLoop, O as Interpolant, P as LineSegments, Q as NearestFilter, R as LinearSRGBColorSpace, S as Group, St as TriangleFanDrawMode, T as InstancedMesh, V as Material, W as Matrix4, X as MeshStandardMaterial, Y as MeshPhysicalMaterial, Z as MirroredRepeatWrapping, _t as SkinnedMesh, a as AnimationMixer, at as PerspectiveCamera, bt as Texture, c as BufferAttribute, ct as PointsMaterial, d as Clock, dt as QuaternionKeyframeTrack, et as NearestMipmapNearestFilter, f as Color, ft as RepeatWrapping, g as DoubleSide, gt as Skeleton, h as DirectionalLight, i as AnimationClip, it as PCFSoftShadowMap, j as Line, k as InterpolateDiscrete, kt as VectorKeyframeTrack, l as BufferGeometry, lt as PropertyBinding, mt as Scene, n as WebGLRenderer, nt as Object3D, o as Bone, ot as PointLight, p as ColorManagement, pt as SRGBColorSpace, r as AmbientLight, rt as OrthographicCamera, s as Box3, st as Points, t as OrbitControls, tt as NumberKeyframeTrack, u as ClampToEdgeWrapping, ut as Quaternion, vt as Sphere, w as InstancedBufferAttribute, wt as TrianglesDrawMode, x as FrontSide, xt as TextureLoader, y as FileLoader, yt as SpotLight, z as Loader } from "./OrbitControls-IxCQ6d1o.js";
	...
	_getWorker(taskID, taskCost) {
		return this._initDecoder().then(() => {
			if (this.workerPool.length < this.workerLimit) {
				const worker$1 = new Worker(this.workerSourceURL);
				worker$1._callbacks = {};
				worker$1._taskCosts = {};
				worker$1._taskLoad = 0;
				...
				worker$1.onmessage = function(e) {
					...
					switch (message.type) {
						case "decode":
							worker$1._callbacks[message.id].resolve(message);
							break;
						case "error":
							worker$1._callbacks[message.id].reject(message);
							break;
						default: console.error("THREE.DRACOLoader: Unexpected message, \"" + message.type + "\"");
					}
				};
				this.workerPool.push(worker$1);
...
function clone(source) {
	...
	const clone$1 = source.clone();
	parallelTraverse(source, clone$1, function(sourceNode, clonedNode) {
		sourceLookup.set(clonedNode, sourceNode);
		cloneLookup.set(sourceNode, clonedNode);
	});
	clone$1.traverse(function(node) {
...
var GLTFLightsExtension = class {
	...
	createNodeAttachment(nodeIndex) {
		...
		return this._loadLight(lightIndex).then(function(light) {
			return parser._getNodeRef(self$1.cache, lightIndex, light);
		});
	}
};
	...
	loadBufferView(index) {
		...
		if (bufferView.extensions && bufferView.extensions[this.name]) {
			...
			return buffer.then(function(res) {
				...
				if (decoder.decodeGltfBufferAsync) return decoder.decodeGltfBufferAsync(count, stride, source, extensionDef.mode, extensionDef.filter).then(function(res$1) {
					return res$1.buffer;
				});
...
var GLTFParser = class {
	...
	_getNodeRef(cache, index, object) {
		...
		const updateMappings = (original, clone$1) => {
			const mappings = this.associations.get(original);
			if (mappings != null) this.associations.set(clone$1, mappings);
			for (const [i, child] of original.children.entries()) updateMappings(child, clone$1.children[i]);
		};
	...
	loadImageSource(sourceIndex, loader) {
		...
		const promise = Promise.resolve(sourceURI).then(function(sourceURI$1) {
			return new Promise(function(resolve, reject) {
				...
				loader.load(LoaderUtils.resolveURL(sourceURI$1, options.path), onLoad, void 0, reject);
			});
		}).then(function(texture) {
			if (isObjectURL === true) URL$1.revokeObjectURL(sourceURI);
	...
	loadScene(sceneIndex) {
		...
		return Promise.all(pending).then(function(nodes) {
			...
			const reduceAssociations = (node) => {
				...
				node.traverse((node$1) => {
					const mappings = parser.associations.get(node$1);
					if (mappings != null) reducedAssociations.set(node$1, mappings);
				});
...
function addPrimitiveAttributes(geometry, primitiveDef, parser) {
	...
	if (primitiveDef.indices !== void 0 && !geometry.index) {
		const accessor = parser.getDependency("accessor", primitiveDef.indices).then(function(accessor$1) {
			geometry.setIndex(accessor$1);
		});
...
var DEFAULT_W = 480;
var DEFAULT_H = 340;
var GLBViewer = ({ src, width, height, onError, onLoad, children }) => {
	...
	(0, import_react.useEffect)(() => {
		...
		const camera = new PerspectiveCamera(45, w / h, .01, 1e3);
		...
		loader.load(src, (gltf) => {
			...
			scene.add(model);
			camera.position.set(0, size.y * scale * .5, maxDim * scale * 1.8);
			camera.lookAt(0, 0, 0);
...
var FallbackBox = dt.div`
  ...
  font-size: 13px;
`;
var GLBViewer_default = GLBViewer;
export { GLBViewer_default as default };
